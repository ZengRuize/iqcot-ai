function rows = iqcot_r049j_pr_ecb_post_active_inhibit_chunk(runSimulink, maxCases, startRow)
%IQCOT_R049J_PR_ECB_POST_ACTIVE_INHIBIT_CHUNK Run active-HS-only chunk.
%
% Dry run:
%   iqcot_r049j_pr_ecb_post_active_inhibit_chunk(false)
%
% True run:
%   iqcot_r049j_pr_ecb_post_active_inhibit_chunk(true)
%
% Scope: same repaired R049I 40A->20A two-offset chunk, but A2 uses a deferred
% request-path post-active inhibit window.  Ton truncation is disabled for both
% A0 and A2.
...
setappdata(0, "iqcot_r049j_run_simulink", runSimulink);
setappdata(0, "iqcot_r049j_max_cases", maxCases);
setappdata(0, "iqcot_r049j_start_row", startRow);

run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");

runSimulink = getappdata(0, "iqcot_r049j_run_simulink");
maxCases = getappdata(0, "iqcot_r049j_max_cases");
startRow = getappdata(0, "iqcot_r049j_start_row");
rmappdata(0, "iqcot_r049j_run_simulink");
rmappdata(0, "iqcot_r049j_max_cases");
rmappdata(0, "iqcot_r049j_start_row");
...
modelFile = iqcot_r049j_build_post_active_inhibit_model();
[~, model] = fileparts(modelFile);

fullPlan = buildPlan();
planPath = fullfile(chunkRoot, "r049j_post_active_inhibit_plan.csv");
...
if ~runSimulink
    ...
    fprintf("R049J_POST_ACTIVE_INHIBIT_PLAN=%s\n", planPath);
    fprintf("Dry run only. To run the post-active inhibit chunk:\n");
    fprintf("  iqcot_r049j_pr_ecb_post_active_inhibit_chunk(true)\n");
    return;
end
...
for k = 1:height(plan)
    ...
    fprintf("R049J case %s: %s, %.1f A -> %.3f A, offset %.3f us, inhibit %.3f..%.3f us\n", ...
    ...
    try
        ...
        warning("iqcot:R049JPostActiveInhibitCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(chunkRoot, "r049j_post_active_inhibit_results_" + runLabel + ".csv");
comparisonPath = fullfile(chunkRoot, "r049j_post_active_inhibit_comparison_" + runLabel + ".csv");
reportPath = fullfile(chunkRoot, "r049j_post_active_inhibit_report_" + runLabel + ".md");
...
fprintf("R049J_POST_ACTIVE_INHIBIT_RESULTS=%s\n", resultPath);
fprintf("R049J_POST_ACTIVE_INHIBIT_COMPARISON=%s\n", comparisonPath);
fprintf("R049J_POST_ACTIVE_INHIBIT_REPORT=%s\n", reportPath);
fprintf("R049J_DECISION=%s\n", decision);
end

function plan = buildPlan()
case_id = [
    "r049j_20A_off0p050_a0";
    "r049j_20A_off0p050_a2_post_active_inhibit";
    "r049j_20A_off0p105_a0";
    "r049j_20A_off0p105_a2_post_active_inhibit"
];
controller = [
    "A0_no_inhibit";
    "A2_deferred_post_active_inhibit";
    "A0_no_inhibit";
    "A2_deferred_post_active_inhibit"
];
...
post_inhibit_window_us = [-0.001; 1.930; -0.001; 1.930];
delta_v_allow_mV = 10 * ones(numel(case_id), 1);
objective = repmat("r049j_deferred_post_active_inhibit", numel(case_id), 1);
role = [
    "baseline_same_model";
    "deferred_post_active_request_inhibit";
    "baseline_same_model";
    "deferred_post_active_request_inhibit"
];
...
postInhibit = valuesAtOptional(logs, "post_active_inhibit", t, zeros(size(t)));
...
wavePath = fullfile(dataRoot, string(spec.case_id) + "_r049j_post_active_inhibit_wave.csv");
wave = table(1e6 * tRel, vout, spec.load_A * ones(numel(t), 1), ...
    ...
    'req_global','allow_after_inhibit','post_active_inhibit','protect_state','phase_idx', ...
...
for k = 1:numel(offsets)
    ...
    a2 = sub(sub.controller == "A2_deferred_post_active_inhibit", :);
...
markBlockOutport(model + "/R049J_post_active_inhibit_double", 1, "post_active_inhibit");
markBlockOutport(model + "/R049J_protect_state_double", 1, "protect_state");
for phase = 1:4
    ...
    markBlockOutport(model + "/R049J_pulse_inhibit" + phase + "_double", 1, "pulse_inhibit" + phase);
end
end
...
reportPath = fullfile(chunkRoot, "r049j_post_active_inhibit_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049J Deferred Post-Active Pulse-Inhibit Dry Run\n\n");
fprintf(fid, "Prepared 40A->20A crossed with two phase offsets.\n\n");
fprintf(fid, "- Derived model: %s\n", modelFile);
fprintf(fid, "- Plan: %s\n", planPath);
fprintf(fid, "- The completed R049I `.slx` model is copied into a new R049J derived file, not modified in place.\n");
fprintf(fid, "- A2 uses request-path `post_active_inhibit` after the baseline active pulse's natural end.\n");
fprintf(fid, "- Ton truncation is disabled in both A0 and A2.\n\n");
fprintf(fid, "Run with `iqcot_r049j_pr_ecb_post_active_inhibit_chunk(true)`.\n");
end

function writeRunReport(rows, comparison, decision, resultPath, comparisonPath, reportPath, modelFile, runLabel)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049J PR-ECB Deferred Post-Active Pulse-Inhibit Chunk\n\n");
fprintf(fid, "Run label: `%s`\n\n", runLabel);
fprintf(fid, "## Scope and hypothesis\n\n");
fprintf(fid, "- Model version: R049J copy of R049I with inherited repaired lower bound and added request-path post-active inhibit gate.\n");
fprintf(fid, "- Diagnostic: `40A -> 20A` at offsets `0.05us` and `0.105us`.\n");
fprintf(fid, "- A2 deferred post-active pulse inhibit: `delay=70ns`, `window=1.93us`; Ton truncation disabled.\n");
...
fprintf(fid, "- Wave snapshots: `output/data/*_r049j_post_active_inhibit_wave.csv`\n\n");
...
fprintf(fid, "\n## Decision\n\n");
fprintf(fid, "```text\n%s\n```\n\n", decision);
fprintf(fid, "This decision applies only to this R049J post-active inhibit chunk.\n");
end
