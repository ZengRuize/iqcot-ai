function rows = iqcot_r049c_pr_ecb_tontrunc_chunk(runSimulink, maxCases, startRow)
%IQCOT_R049C_PR_ECB_TONTRUNC_CHUNK Run the minimal Ton-truncation chunk.
%
% Dry run:
%   iqcot_r049c_pr_ecb_tontrunc_chunk(false)
%
% True run:
%   iqcot_r049c_pr_ecb_tontrunc_chunk(true)
%
% Scope: one load-drop magnitude (40A -> near0 represented by 1A) crossed with
% two phase offsets (0.05us active-HS boundary and 0.105us post-turnoff
% reference).  For each offset, run A0 same-model no-trunc and A2 minimal
% command-path Ton truncation in the R049C derived model.
...
setappdata(0, "iqcot_r049c_run_simulink", runSimulink);
setappdata(0, "iqcot_r049c_max_cases", maxCases);
setappdata(0, "iqcot_r049c_start_row", startRow);

run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");

runSimulink = getappdata(0, "iqcot_r049c_run_simulink");
maxCases = getappdata(0, "iqcot_r049c_max_cases");
startRow = getappdata(0, "iqcot_r049c_start_row");
rmappdata(0, "iqcot_r049c_run_simulink");
rmappdata(0, "iqcot_r049c_max_cases");
rmappdata(0, "iqcot_r049c_start_row");
...
modelFile = iqcot_r049c_build_tontrunc_model();
[~, model] = fileparts(modelFile);

fullPlan = buildR049CPlan();
planPath = fullfile(chunkRoot, "r049c_tontrunc_minimal_plan.csv");
...
if ~runSimulink
    ...
    fprintf("R049C_TONTRUNC_PLAN=%s\n", planPath);
    fprintf("Dry run only. To run the minimal chunk:\n");
    fprintf("  iqcot_r049c_pr_ecb_tontrunc_chunk(true)\n");
    return;
end
...
rows = table();
for k = 1:height(plan)
    spec = makeR049CSpec(plan(k, :), common);
    fprintf("R049C case %s: %s, %.1f A -> %.3f A, offset %.3f us, threshold %.3f mV\n", ...
    ...
    try
        ...
        warning("iqcot:R049CTonTruncCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(chunkRoot, "r049c_tontrunc_minimal_results_" + runLabel + ".csv");
comparisonPath = fullfile(chunkRoot, "r049c_tontrunc_minimal_comparison_" + runLabel + ".csv");
reportPath = fullfile(chunkRoot, "r049c_tontrunc_minimal_report_" + runLabel + ".md");
...
fprintf("R049C_TONTRUNC_RESULTS=%s\n", resultPath);
fprintf("R049C_TONTRUNC_COMPARISON=%s\n", comparisonPath);
fprintf("R049C_TONTRUNC_REPORT=%s\n", reportPath);
fprintf("R049C_DECISION=%s\n", decision);
end

function plan = buildR049CPlan()
case_id = [
    "r049c_near0_off0p050_a0";
    "r049c_near0_off0p050_a2_tontrunc";
    "r049c_near0_off0p105_a0";
    "r049c_near0_off0p105_a2_tontrunc"
];
controller = [
    "A0_no_trunc";
    "A2_ton_trunc";
...
objective = repmat("r049c_minimal_tontrunc", numel(case_id), 1);
role = [
    "baseline_same_model";
    "minimal_ton_truncation";
    "baseline_same_model";
    "minimal_ton_truncation"
];
...
function spec = makeR049CSpec(planRow, common)
spec = struct();
...
il = zeros(numel(t), 4);
qh = zeros(numel(t), 4);
tonRaw = zeros(numel(t), 4);
tonCmd = zeros(numel(t), 4);
for phase = 1:4
    il(:, phase) = valuesAt(logs, "il" + phase, t);
    qh(:, phase) = valuesAt(logs, "qh" + phase, t);
    tonRaw(:, phase) = valuesAtOptional(logs, "ton_iqcot_raw" + phase, t, NaN(size(t)));
...
wavePath = fullfile(dataRoot, string(spec.case_id) + "_r049c_tontrunc_wave.csv");
Iload = spec.load_A * ones(numel(t), 1);
wave = table(1e6 * tRel, vout, Iload, il(:,1), il(:,2), il(:,3), il(:,4), ...
    qh(:,1), qh(:,2), qh(:,3), qh(:,4), truncGlobal, protectState, phaseIdx, ...
    tonRaw(:,1), tonRaw(:,2), tonRaw(:,3), tonRaw(:,4), ...
    tonCmd(:,1), tonCmd(:,2), tonCmd(:,3), tonCmd(:,4), iphRef, ...
    'VariableNames', {'time_from_load_step_us','vout_V','iload_A','il1_A','il2_A','il3_A','il4_A', ...
    'qh1','qh2','qh3','qh4','ton_trunc_global','protect_state','phase_idx', ...
    'ton_raw1_s','ton_raw2_s','ton_raw3_s','ton_raw4_s', ...
...
if all(comparison.a2_trunc_duration_us <= 0)
    decision = "IMPLEMENTATION_ISSUE";
    return;
end
...
if ~isempty(activeRow) && activeRow.peak_improvement_mV >= 0.05
...
for phase = 1:4
    ...
    markBlockOutport(model + "/IQCOT_Ton_Adapter", phase, "ton_iqcot_raw" + phase);
...
reportPath = fullfile(chunkRoot, "r049c_tontrunc_minimal_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049C Ton-Truncation Minimal Chunk Dry Run\n\n");
fprintf(fid, "Prepared one load-drop magnitude crossed with two phase offsets.\n\n");
fprintf(fid, "- Derived model: %s\n", modelFile);
fprintf(fid, "- Plan: %s\n", planPath);
fprintf(fid, "- No original, R049A, or R049B `.slx` model is modified.\n\n");
fprintf(fid, "Run with `iqcot_r049c_pr_ecb_tontrunc_chunk(true)`.\n");
end

function writeRunReport(rows, comparison, decision, resultPath, comparisonPath, reportPath, modelFile, runLabel)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049C PR-ECB Minimal Ton-Truncation Chunk\n\n");
fprintf(fid, "Run label: `%s`\n\n", runLabel);
fprintf(fid, "## Scope and hypothesis\n\n");
fprintf(fid, "- Model version: GAE-IQCOT R047 + R049A scaffold + R049C command-path Ton truncation.\n");
fprintf(fid, "- Hypothesis: shortening the COT-cell Ton command during the first cut-load window reduces the first peak for the active-HS offset and does not worsen the post-turnoff offset.\n");
fprintf(fid, "- Expected failure mode: the COT cell may sample Ton only at pulse start, so command-path truncation may not terminate an already-active high-side pulse.\n");
fprintf(fid, "- Metrics: peak overshoot, first-peak time, truncation duration, Ton-min hit count, secondary undershoot, final error.\n");
...
fprintf(fid, "- Wave snapshots: `output/data/*_r049c_tontrunc_wave.csv`\n\n");
fprintf(fid, "## Per-case results\n\n");
fprintf(fid, "| case | ctrl | offset us | peak mV | t_peak us | trunc us | trunc edges | rem Ton4 ns | undershoot mV | final mV |\n");
fprintf(fid, "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|\n");
for k = 1:height(rows)
    if rows.success(k)
        fprintf(fid, "| %s | %s | %.3f | %.4f | %.4f | %.4f | %.0f | %.4f | %.4f | %.4f |\n", ...
        ...
        fprintf(fid, "| %s | %s | %.3f | failed | failed | failed | failed | failed | failed | failed |\n", ...
            rows.case_id(k), rows.controller(k), rows.load_step_offset_us(k));
    end
end
...
if isempty(comparison)
    ...
    fprintf(fid, "| offset us | A0 peak mV | A2 peak mV | improvement mV | A2 trunc us | A2 Ton-min hits | A2-A0 undershoot mV |\n");
    fprintf(fid, "|---:|---:|---:|---:|---:|---:|---:|\n");
    for k = 1:height(comparison)
        fprintf(fid, "| %.3f | %.4f | %.4f | %.4f | %.4f | %.0f | %.4f |\n", ...
            ...
            comparison.a2_minus_a0_undershoot_mV(k));
    end
end
fprintf(fid, "\n## Decision\n\n");
fprintf(fid, "```text\n%s\n```\n\n", decision);
fprintf(fid, "This decision applies only to this minimal R049C chunk.\n");
end
