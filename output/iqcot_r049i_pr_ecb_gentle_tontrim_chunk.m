function rows = iqcot_r049i_pr_ecb_gentle_tontrim_chunk(runSimulink, maxCases, startRow)
%IQCOT_R049I_PR_ECB_GENTLE_TONTRIM_CHUNK Run active-HS-only chunk.
%
% Dry run:
%   iqcot_r049i_pr_ecb_gentle_tontrim_chunk(false)
%
% True run:
%   iqcot_r049i_pr_ecb_gentle_tontrim_chunk(true)
%
% Scope: same repaired R049G 40A->20A two-offset chunk, but A2 uses a gentler
% phase-selective Ton floor (120 ns) instead of hard 5 ns Ton-min.
...
setappdata(0, "iqcot_r049i_run_simulink", runSimulink);
setappdata(0, "iqcot_r049i_max_cases", maxCases);
setappdata(0, "iqcot_r049i_start_row", startRow);

run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");

runSimulink = getappdata(0, "iqcot_r049i_run_simulink");
maxCases = getappdata(0, "iqcot_r049i_max_cases");
startRow = getappdata(0, "iqcot_r049i_start_row");
rmappdata(0, "iqcot_r049i_run_simulink");
rmappdata(0, "iqcot_r049i_max_cases");
rmappdata(0, "iqcot_r049i_start_row");
...
modelFile = iqcot_r049i_build_gentle_tontrim_model();
[~, model] = fileparts(modelFile);

fullPlan = buildPlan();
planPath = fullfile(chunkRoot, "r049i_gentle_tontrim_plan.csv");
...
if ~runSimulink
    ...
    fprintf("R049I_GENTLE_TONTRIM_PLAN=%s\n", planPath);
    fprintf("Dry run only. To run the phase-selective chunk:\n");
    fprintf("  iqcot_r049i_pr_ecb_gentle_tontrim_chunk(true)\n");
    return;
end
...
for k = 1:height(plan)
    ...
    fprintf("R049I case %s: %s, %.1f A -> %.3f A, offset %.3f us, window %.3f us\n", ...
    ...
    try
        ...
        warning("iqcot:R049IGentleTonTrimCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(chunkRoot, "r049i_gentle_tontrim_results_" + runLabel + ".csv");
comparisonPath = fullfile(chunkRoot, "r049i_gentle_tontrim_comparison_" + runLabel + ".csv");
reportPath = fullfile(chunkRoot, "r049i_gentle_tontrim_report_" + runLabel + ".md");
...
fprintf("R049I_GENTLE_TONTRIM_RESULTS=%s\n", resultPath);
fprintf("R049I_GENTLE_TONTRIM_COMPARISON=%s\n", comparisonPath);
fprintf("R049I_GENTLE_TONTRIM_REPORT=%s\n", reportPath);
fprintf("R049I_DECISION=%s\n", decision);
end

function plan = buildPlan()
case_id = [
    "r049i_20A_off0p050_a0";
    "r049i_20A_off0p050_a2_gentle_trim";
    "r049i_20A_off0p105_a0";
    "r049i_20A_off0p105_a2_gentle_trim"
];
controller = [
    "A0_no_trunc";
    "A2_gentle_phase_selective_ton_trim";
    "A0_no_trunc";
    "A2_gentle_phase_selective_ton_trim"
];
...
tton_trunc_min_ns = [196.5; 120.0; 196.5; 120.0];
tton_trunc_window_us = [-0.001; 0.080; -0.001; 0.080];
delta_v_allow_mV = 10 * ones(numel(case_id), 1);
objective = repmat("r049i_gentle_phase_selective_tontrim", numel(case_id), 1);
role = [
    "baseline_same_model";
    "gentle_phase_selective_active_hs_ton_trim";
    "baseline_same_model";
    "gentle_phase_selective_active_hs_ton_trim"
];
...
wavePath = fullfile(dataRoot, string(spec.case_id) + "_r049i_gentle_tontrim_wave.csv");
...
for k = 1:numel(offsets)
    ...
    a2 = sub(sub.controller == "A2_gentle_phase_selective_ton_trim", :);
...
reportPath = fullfile(chunkRoot, "r049i_gentle_tontrim_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049I Gentle Phase-Selective Ton-Trim Dry Run\n\n");
fprintf(fid, "Prepared 40A->20A crossed with two phase offsets.\n\n");
fprintf(fid, "- Derived model: %s\n", modelFile);
fprintf(fid, "- Plan: %s\n", planPath);
fprintf(fid, "- The completed R049G `.slx` model is copied into a new R049I derived file, not modified in place.\n");
fprintf(fid, "- Inherits the repaired `R049C_After_LoadStep` lower bound and per-phase guard.\n");
fprintf(fid, "- A2 uses `ton_trim_i = early_window AND qh_i` with `Tton_trunc_min=120ns`.\n\n");
fprintf(fid, "Run with `iqcot_r049i_pr_ecb_gentle_tontrim_chunk(true)`.\n");
end

function writeRunReport(rows, comparison, decision, resultPath, comparisonPath, reportPath, modelFile, runLabel)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049I PR-ECB Gentle Phase-Selective Ton-Trim Chunk\n\n");
fprintf(fid, "Run label: `%s`\n\n", runLabel);
fprintf(fid, "## Scope and hypothesis\n\n");
fprintf(fid, "- Model version: R049I copy of R049G with repaired `t_load_step` lower bound and inherited per-phase `early_window AND qh_i` guards.\n");
fprintf(fid, "- Diagnostic: `40A -> 20A` at offsets `0.05us` and `0.105us`.\n");
fprintf(fid, "- A2 gentle phase-selective Ton trim: `Tton_trunc_min=120ns`, `Tton_trunc_window=80ns`.\n");
...
fprintf(fid, "- Wave snapshots: `output/data/*_r049i_gentle_tontrim_wave.csv`\n\n");
...
fprintf(fid, "\n## Decision\n\n");
fprintf(fid, "```text\n%s\n```\n\n", decision);
fprintf(fid, "This decision applies only to this R049I gentle Ton-trim chunk.\n");
end
