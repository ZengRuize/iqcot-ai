# 四相数字 IQCOT Buck 变换器的积分事件核、相索引盐跃小信号建模与 PR-ECB 大信号第一峰风险边界校准

> v8 版本说明：本稿在 v7 的 IEK/PIS-IEK 小信号与数字实现预算证据链上，增量整合 R039--R043 的 PR-ECB 大信号第一峰风险边界研究。本文仍只主张 derived-Simulink/offline evidence；不声称硬件或 HIL 验证，不声称全局 `T_slew` 最优，也不声称 AI 替代 IQCOT 内环。
...
## 20. v8 新增：PR-ECB 大信号第一峰风险边界

前述 IEK/PIS-IEK 章节解决的是四相数字 IQCOT 在正常或准正常事件序列附近的可线性化小信号问题：面积阈值、相索引、reset、检测延迟和 Ton_i 执行量如何进入 event-to-event Jacobian，以及哪些执行量适合相位调节或 DC 均流。对于 40 A 级大切载，第一峰往往发生在监督层参考斜率动作真正影响功率级之前。若直接要求 PIS-IEK 精确预测这类大信号第一峰，会把小信号模型用到不该用的位置。因此，v8 将大信号第一峰单独写成 PR-ECB（phase-resolved energy-charge boundary）风险边界特征，而把 PIS-IEK、r_hat 和 B_epsilon^sw 保留为峰后事件恢复、skip/reentry、相位间隔和 T_slew 部署的模型。

PR-ECB 的输入是负载阶跃瞬间的相电感电流、门极高边状态、输出电容、ESR、电感和允许电压峰值。它不输出 gate command，也不替代 IQCOT 内环；它只给出第一峰风险特征和保守边界。R039 首先在共享的 40 A -> 20 A 负载切换事件上验证该分离思想。五个 delayed-reference derived-Simulink 工况覆盖 R038 的局部锚点 46/50/54 us 和 tau_AI=2 us 下的 30/48 us 近 ties。结果显示，这五行的第一峰状态相同，因为第一峰在延迟参考动作影响功率级之前出现。R039 的第一版边界为：energy estimate 4.349633 mV、charge+ESR estimate 3.903338 mV、derived-Simulink 第一峰 2.235008 mV，在 Delta V_allow=10 mV 下 r_E=0.434963。该结果支持 PR-ECB 与 PIS-IEK 职责分离，但不能写成硬件验证。

R040 进一步改变负载阶跃相位和负载幅度，说明 R039 的保守系数不能作为常数。40 A -> 20 A 中，load-step offset 从 0 到 0.375 us 时，energy-bound estimate 从 4.085 到 5.649 mV，r_E 从 0.409 到 0.565，而实际 derived-Simulink 第一峰集中在 2.134 到 2.244 mV。40 A -> 10 A 的 r_E 为 0.587-0.678；40 A -> near0 为 0.858-0.993，且 charge+ESR 成为 dominant bound。特别是 near0 offset-0 行显示 energy-only 低于实际第一峰，而 charge+ESR 仍保守，因此 PR-ECB 必须保留 max(energy, charge+ESR) 或分段 dominant-bound 选择。

R041 测试 remaining high-side on-time correction E_HS,rem。它从 R040 八行中反推 L=0.2 uH 与 Cout=7.26 mF，只在负载阶跃发生时仍有高边导通相的行上加入剩余高边能量。非零 E_HS,rem 只出现在三个 offset-0 行，均为 phase 4 剩余约 102 ns 高边 on-time。该修正能修复 near0 offset-0 的 energy-only 低估：energy/actual 从 0.876x 变为 corrected-energy/actual 1.169x。但原始 max(energy, charge+ESR) 已经在八行中全部保守；直接把 E_HS,rem 加进 max-bound 会提高 active-HS 20 A 与 10 A 行的保守性。因此，R041 的安全结论是：E_HS,rem 是 phase-state feature，而不是全局加性律。

R042 围绕 phase-4 高边关断边界进行 phase-dense 验证。near0/5A/10A/20A 与 offsets 0.05/0.09/0.105/0.125/0.20 us 交叉得到 20/20 行 derived-Simulink 结果。四个负载目标都显示一致边界：0.05 us 时剩余高边 on-time 为 52 ns，0.09 us 时为 12 ns，从 0.105 us 开始为 0 ns。E_HS,rem 在 8/20 行中非零，并从约 7.885 uJ 衰减到 1.961 uJ 后消失。最终负载分段结果为：near0 r_E(max corrected)=0.895-0.983，5A 为 0.760-0.839，10A 为 0.619-0.705，20A 为 0.516-0.602。dominant-bound 随负载幅度变化：near0/5A 由 charge+ESR 主导，10A/20A 多数由 corrected-energy/raw energy 主导。

## 21. R043 分段 PR-ECB 校准面

R043 将 R041 修正后的 R040 行与完整 R042 phase-dense 行合并为 28 行 row-level 证据，并拟合离线 segmented PR-ECB calibration surface。分段变量包括 load-drop magnitude、active high-side remaining-on-time 和 dominant-bound class。表 8 给出可写入论文或答辩的紧凑规则表；其中 conservative band 是 recommended-bound/actual range 向外按 0.05x 取整后的保守摘要，不能解释为全局硬件安全系数。

| segment | active HS | target class | recommended bound | r_E range | bound/actual range | conservative band | claim boundary |
|---|---:|---|---|---:|---:|---:|---|
| high_drop_charge_esr | 0 | near0/5A | charge+ESR | 0.760-0.968 | 1.625-1.701 | 1.60-1.75x | post-turnoff large cut-loads use charge+ESR |
| high_drop_charge_esr | 1 | near0/5A | charge+ESR | 0.832-0.993 | 1.522-1.665 | 1.50-1.70x | active-HS remains diagnostic; charge+ESR still dominates |
| mid_drop_transition | 0 | 10A | raw energy | 0.587-0.683 | 1.815-1.853 | 1.80-1.90x | post-turnoff transition rows use raw energy |
| mid_drop_transition | 1 | 10A | corrected energy | 0.686-0.729 | 1.737-1.792 | 1.70-1.80x | active-HS transition rows use corrected energy |
| low_drop_energy | 0 | 20A | raw energy | 0.409-0.580 | 1.820-2.810 | 1.80-2.85x | smaller cut-loads are energy dominated but conservative |
| low_drop_energy | 1 | 20A | corrected energy | 0.583-0.626 | 2.802-2.870 | 2.80-2.90x | active-HS smaller cut-loads use corrected energy with high conservatism |

这个表给出的最重要结论不是某一个比例常数，而是分段结构本身。near0/5A-like 大切载中，charge+ESR 对第一峰风险更安全；10A-like 行处在 transition band，active-HS 状态决定 corrected energy 是否有必要；20A-like 较小切载中，energy/corrected-energy 是 dominant family，但相对实际第一峰保守度更高。E_HS,rem 只参与 active-HS segmentation，不应写成所有负载和所有相位都可叠加的能量修正项。

因此，v8 建议把完整监督层写成两层风险接口：第一层 PR-ECB 在负载阶跃发生后立即给出第一峰边界和 conservative band；第二层 PIS-IEK/r_hat/B_epsilon^sw 在事件序列演化中负责 skip、settling、phase-spacing 和 T_slew 候选动作的安全投影。AI 监督层只能在这个接口上调度低维参数，不能逐脉冲替代 IQCOT 内环。

## 22. v8 证据矩阵与可允许表述

| 论断 | 证据 | 强度 | 可允许写法 | 禁止写法 |
|---|---|---|---|---|
| PIS-IEK 可用于四相 IQCOT 小信号执行量分类 | IEK/PIS-IEK 局部 Jacobian、幅值扫描、lifted frequency response、Simulink 有限差分和 Monte Carlo 预算 | 中高；模型内证据完整 | PIS-IEK 支持事件域执行量分类和数字预算 | PIS-IEK 精确预测所有大信号第一峰 |
| Lambda_diff 不是强 DC 均流执行量 | 解析执行量矩阵、面积触发副本、逐相 IEK 副本和 trim 版有限差分均显示 Lambda_m2 电流通道弱 | 中高 | Lambda_diff 更适合 phase-spacing/ripple-cancellation 微调 | Lambda_diff 对电流完全无影响 |
| Ton_diff 是强 DC 均流执行量但有相位代价 | 解析矩阵与 Simulink 有限差分均显示 Ton_m2 电流通道显著强于 Lambda_m2 | 中高 | Ton_diff 是主要均流通道，需要相位约束 | 只调 Ton_diff 即可解决所有瞬态问题 |
| PR-ECB 是第一峰风险边界特征 | R039--R043 共 33 行 first-peak/校准相关 derived-Simulink 与离线后处理证据 | 中等；仍为 derived model | PR-ECB 可作为监督层第一峰风险特征和安全边界 | PR-ECB 已完成硬件/HIL 验证 |
| R043 支持分段校准而非全局加性律 | 28 行 R043 合并证据、6 条分段规则、所有 recommended bound/actual 均大于 1.52x | 中等 | 按 load-drop 与 active-HS 状态选择 dominant bound | E_HS,rem 是全局可加 correction law |
| AI 只应作为监督层参数调度 | R027--R038 的 delayed-reference 与 risk-interface 证据显示监督层可给候选和安全投影，但需要 dense fallback | 中等 | AI 输出候选、风险和投影后的低维参数 | AI 直接替代 IQCOT gate command 或内环 |

## 23. v8 审稿式自查与仍需补强项

| 风险 | v8 处理 | 剩余限制 |
|---|---|---|
| 论文主线过宽 | v8 将小信号 IEK/PIS-IEK 与大信号 PR-ECB 明确分层：前者用于 event-to-event 小信号，后者用于第一峰边界 | 若投 IEEE TPEL/JESTPE，建议拆成小信号模型论文和大信号监督层短文 |
| PR-ECB 证据仍是后处理 | R039--R043 明确写成 derived-Simulink/offline evidence | 需要 HIL、PLECS 或硬件波形才能写硬件安全 |
| R043 分段规则可能过拟合当前模型 | R043 只给 observed conservative band，未写成 universal factor | 后续应做新 load-drop magnitude 和不同 Vin/Cout/ESR/L hold-out |
| PIS-IEK 与 PR-ECB 职责可能混淆 | v8 明确 PIS-IEK 不预测第一峰，PR-ECB 不处理峰后恢复 | 需要图示化两层接口，避免审稿人误读 |
| 引用和格式仍未完成投稿级审计 | v7 参考文献有核心来源，v8 未新增外部 PR-ECB 文献检索 | 投稿前必须做 citation audit 和 number claim audit |

## 24. v8 数据与脚本补充清单

| 类型 | 文件 |
|---|---|
| PR-ECB R039 runner/postprocess | output/iqcot_r039_pr_ecb_large_signal_probe.m; output/iqcot_r039_pr_ecb_postprocess.py |
| PR-ECB R040 phase/load | output/iqcot_r040_pr_ecb_phase_load_calibration.m; output/iqcot_r040_pr_ecb_phase_load_results_combined.csv |
| PR-ECB R041 remaining high-side correction | output/iqcot_r041_pr_ecb_hsrem_correction.py; output/iqcot_r041_pr_ecb_hsrem_results.csv |
| PR-ECB R042 phase-dense validation | output/iqcot_r042_pr_ecb_phase_dense_calibration.m; output/iqcot_r042_pr_ecb_phase_dense_results_combined.csv; output/iqcot_r042_pr_ecb_phase_dense_summary.csv |
| PR-ECB R043 segmented calibration | output/iqcot_r043_pr_ecb_segmented_calibration.py; output/iqcot_r043_pr_ecb_segmented_rows.csv; output/iqcot_r043_pr_ecb_segmented_rules.csv; output/iqcot_r043_pr_ecb_segmented_report.md |
| v8 证据矩阵更新 | output/iqcot_claims_evidence_matrix.md; research-wiki/experiments/pr-ecb-segmented-r043.md; refine-logs/LOCAL_AUDIT_R043_PR_ECB_SEGMENTED_CALIBRATION_20260622.md |

## 25. v8 结论补充

v8 后，本文可更清楚地区分三个层次。第一，IEK/PIS-IEK 给出四相数字 IQCOT 的事件域小信号模型，使面积阈值、相索引、积分 reset、检测延迟和 Ton_i 执行量能够进入同一个可计算框架。第二，执行量证据链表明 Lambda_diff 和 Ton_diff 不能混用：前者主要影响相位/纹波，后者才是主要 DC 均流通道但带来相位代价。第三，PR-ECB 补上大切载第一峰风险边界，但它必须写成分段监督特征，而不是硬件安全证明或全局校准律。

因此，本文最终主张应保持克制：四相数字 IQCOT 的监督层可以利用 IEK/PIS-IEK 进行小信号执行量分类，利用 PR-ECB 进行第一峰风险筛选，再通过 r_hat/B_epsilon^sw 做候选动作安全投影。当前证据足以支撑一篇严谨的模型化和 derived-Simulink 验证论文；但若要写成硬件性能提升论文，还必须补充真实控制器、驱动、PCB 寄生、测量链路和 HIL/实物验证。
