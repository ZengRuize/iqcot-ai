"""R049Q waveform audit for the 1.630us release-later-point test."""
...
RESULTS = OUT / "r049q_release_later_point_results_full.csv"
...
CASES = [
    ("0.050", "A0", "baseline", "r049q_20A_off0p050_a0_r049q_release_later_point_wave.csv"),
    ("0.050", "A2_1p630", "release_1p630us", "r049q_20A_off0p050_a2_rel1p630_r049q_release_later_point_wave.csv"),
    ("0.105", "A0", "baseline", "r049q_20A_off0p105_a0_r049q_release_later_point_wave.csv"),
    ("0.105", "A2_1p630", "release_1p630us", "r049q_20A_off0p105_a2_rel1p630_r049q_release_later_point_wave.csv"),
]
...
def pair_rows(rows):
    ...
    for offset in ["0.050", "0.105"]:
        for window, _, _ in WINDOWS:
            ...
            a2 = by_key.get((offset, window, "A2_1p630"))
...
def write_report(pair_deltas, baseline_failures, dec, path):
    ...
    lines += [
        ...
        f"```text\n{dec}\n```\n",
        "\n## Interpretation\n",
        "`1.630us` is the first slightly later point after R049P's `1.600us` "
        "midpoint. It should be interpreted only relative to the R049O/R049P/R049N "
        "timing bracket: `1.250-1.450us` was transparent, `1.600us` was "
        "offset-selective, and `1.685us` was active but too hard.\n",
    ]
...
def main():
    ...
    case_path = OUT / "r049q_waveform_metric_case_windows.csv"
    pair_path = OUT / "r049q_waveform_metric_pair_delta.csv"
    report_path = OUT / "r049q_waveform_metric_summary.md"
    write_csv(case_path, case_rows)
    write_csv(pair_path, pair_deltas)
    write_report(pair_deltas, baseline_failures, dec, report_path)
    print(f"R049Q_CASE_WINDOWS={case_path}")
    print(f"R049Q_PAIR_DELTA={pair_path}")
    print(f"R049Q_REPORT={report_path}")
    print(f"R049Q_DECISION={dec}")
