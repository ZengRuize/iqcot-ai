...
| C7 | `Iph_ref` 参考斜率是有实际价值的低维调度变量 | 15 个开关级 Simulink 工况；当前网格 `40 us` 为三个切载深度最佳折中；`near-0A` 欠压从 `35.750 mV` 降至 `10.897 mV` | 强 | “当前扫描网格中，`40 us` 给出最佳折中。” | “`40 us` 是全局最优。” |
...
| `10.897 mV` near-0A `40 us` 欠压 | `E:/Desktop/codex/output/iqcot_dynamic_ref_slew_best_summary.csv` | 当前扫描网格最佳 |
| `-0.569 mV` near-0A `40 us` final error | `E:/Desktop/codex/output/iqcot_dynamic_ref_slew_best_summary.csv` | 不能写成零误差 |
...
| `40 us` best scanned slew | `E:/Desktop/codex/output/iqcot_dynamic_ref_slew_best_summary.csv` | 只能说当前网格 |
...
回答：对“参考斜率”这一局部结论，已有 15 个开关级 Simulink 工况；对“延迟建模必要性”，已有 15360 episode surrogate；对基础 PIS-IEK，小信号层已有 77+80 个结构化工况。仍需补充的是更密集的 `T_slew` 网格、开关级 AI-in-the-loop 和硬件/HIL 验证。
