function rows = iqcot_r040_pr_ecb_phase_load_calibration(runSimulink, maxCases, startRow)
%IQCOT_R040_PR_ECB_PHASE_LOAD_CALIBRATION Calibrate PR-ECB across load-step phase and magnitude.
%
% Dry run:
%   iqcot_r040_pr_ecb_phase_load_calibration(false)
%
% Small true run:
%   iqcot_r040_pr_ecb_phase_load_calibration(true, 5, 1)
%
...
setappdata(0, "iqcot_r040_run_simulink", runSimulink);
setappdata(0, "iqcot_r040_max_cases", maxCases);
setappdata(0, "iqcot_r040_start_row", startRow);

run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");

runSimulink = getappdata(0, "iqcot_r040_run_simulink");
maxCases = getappdata(0, "iqcot_r040_max_cases");
startRow = getappdata(0, "iqcot_r040_start_row");
rmappdata(0, "iqcot_r040_run_simulink");
rmappdata(0, "iqcot_r040_max_cases");
rmappdata(0, "iqcot_r040_start_row");
...
fullPlan = buildR040Plan();
planPath = fullfile(outputRoot, "iqcot_r040_pr_ecb_phase_load_plan.csv");
...
if ~runSimulink
    ...
    fprintf("R040_PR_ECB_PLAN=%s\n", planPath);
    fprintf("Dry run only. To run a small true run:\n");
    fprintf("  iqcot_r040_pr_ecb_phase_load_calibration(true, 5, 1)\n");
    return;
end
...
rows = table();
for k = 1:height(plan)
    spec = makeR040Spec(plan(k, :), common);
    fprintf("R040 case %s: %.1f A -> %.3f A, offset %.3f us, slew %.3f us, delay %.3f us\n", ...
    ...
    try
        ...
        warning("iqcot:R040PrEcbCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(outputRoot, "iqcot_r040_pr_ecb_phase_load_results_" + runLabel + ".csv");
reportPath = fullfile(outputRoot, "iqcot_r040_pr_ecb_phase_load_report_" + runLabel + ".md");
writetable(rows, resultPath);
writeRunReport(rows, resultPath, reportPath, runLabel);
fprintf("R040_PR_ECB_RESULTS=%s\n", resultPath);
fprintf("R040_PR_ECB_REPORT=%s\n", reportPath);
end

function plan = buildR040Plan()
case_id = [
    "r040_20A_off0p000";
    "r040_20A_off0p125";
    "r040_20A_off0p250";
...
target_load_A = [20; 20; 20; 20; 10; 10; 1; 1];
load_drop_A = base_load_A - target_load_A;
load_step_offset_us = [0; 0.125; 0.250; 0.375; 0; 0.250; 0; 0.250];
tau_ai_us = [1.25; 1.25; 1.25; 1.25; 1.25; 1.25; 1.25; 1.25];
selected_ref_slew_us = [46; 46; 46; 46; 60; 60; 60; 60];
delta_v_allow_mV = 10 * ones(numel(case_id), 1);
objective = repmat("pr_ecb_calibration", numel(case_id), 1);
role = [
    "phase_baseline";
    "phase_offset";
    "phase_offset";
...
function spec = makeR040Spec(planRow, common)
spec = struct();
...
wavePath = fullfile(dataRoot, string(spec.case_id) + "_r040_pr_ecb_wave.csv");
...
reportPath = fullfile(outputRoot, "iqcot_r040_pr_ecb_phase_load_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid));
fprintf(fid, "# R040 PR-ECB Phase/Load Calibration Dry Run\n\n");
fprintf(fid, "Prepared phase-offset and load-magnitude calibration plan.\n\n");
fprintf(fid, "- Plan: %s\n", planPath);
...
fprintf(fid, "Run a small batch with iqcot_r040_pr_ecb_phase_load_calibration(true, 5, 1).\n");
end

function writeRunReport(rows, resultPath, reportPath, runLabel)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid));
fprintf(fid, "# R040 PR-ECB Phase/Load Calibration\n\n");
fprintf(fid, "Run label: %s\n\n", runLabel);
fprintf(fid, "- Results: %s\n", resultPath);
fprintf(fid, "- Wave snapshots: output/data/*_r040_pr_ecb_wave.csv\n\n");
