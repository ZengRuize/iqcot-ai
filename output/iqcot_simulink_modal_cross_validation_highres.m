function results = iqcot_simulink_modal_cross_validation_highres()
%IQCOT_SIMULINK_MODAL_CROSS_VALIDATION_HIGHRES
...
csvPath = fullfile(outputRoot, "iqcot_simulink_modal_cross_validation_highres_summary.csv");
writetable(rows, csvPath);

jsonPath = fullfile(outputRoot, "iqcot_simulink_modal_cross_validation_highres_detail.json");
...
exportgraphics(fig, fullfile(figureRoot, "fig12_simulink_modal_cross_validation_highres.png"), Resolution=180);
end
...
reportPath = fullfile(outputRoot, "iqcot_simulink_modal_cross_validation_highres_report.md");
fid = fopen(reportPath, "w", "n", "UTF-8");
cleanup = onCleanup(@() fclose(fid));

fprintf(fid, "# Simulink 四相 IQCOT/COT 模态交叉验证（精选高分辨率）\n\n");
...
fprintf(fid, "## 文件\n\n");
fprintf(fid, "- `iqcot_simulink_modal_cross_validation_highres_summary.csv`\n");
fprintf(fid, "- `iqcot_simulink_modal_cross_validation_highres_detail.json`\n");
fprintf(fid, "- `figures/fig12_simulink_modal_cross_validation_highres.png`\n");
end
