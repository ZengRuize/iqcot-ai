...
def pick(rows, **kwargs):
    for r in rows:
        if all(str(r[k]) == str(v) for k, v in kwargs.items()):
            return r
...
def make_markdown():
    summary = read_csv("iqcot_four_phase_dense_summary.csv")[0]
    matrix = read_csv("iqcot_four_phase_actuator_matrix.csv")
    bit_budget = read_csv("iqcot_digital_area_bit_budget.csv")
    clock_budget = read_csv("iqcot_digital_detection_clock_budget.csv")
    ton_budget = read_csv("iqcot_digital_ton_resolution_budget.csv")
    combined = read_csv("iqcot_digital_combined_jitter_budget.csv")

    lam_m2 = pick(matrix, actuator="lambda_diff", pattern="m2_alt")
    ton_m2 = pick(matrix, actuator="ton_diff", pattern="m2_alt")
    delay_m2 = pick(matrix, actuator="delay_diff", pattern="m2_alt")
    bit12 = pick(bit_budget, area_bits="12")
    clk1 = pick(clock_budget, detect_clock_ns="1.0")
    ton10 = pick(ton_budget, ton_resolution_ps="10")
    comb_pick = [
        r for r in combined
        if r["area_bits"] == "12" and r["detect_clock_ns"] == "1.0" and r["ton_resolution_ps"] == "10"
    ][0]

    md = """# 四相数字 IQCOT Buck 变换器的积分事件核小信号建模与受限均流执行量设计
...
四相模型参数为：`Vin={summary['Vin']} V`、`Vref={summary['Vref']} V`、`Iout={summary['Iout']} A`、每相电流 `Iph={summary['Iph']} A`、`L={float(summary['L'])*1e9:.0f} nH`、`C={float(summary['C'])*1e3:.1f} mF`、`DCR={float(summary['DCR'])*1e3:.1f} mΩ`、`Ri={float(summary['Ri'])*1e3:.1f} mΩ`、每相开关频率 `500 kHz`。稳态 `Ton={fnum(summary['Ton_ns'], 3)} ns`，`Twait={fnum(summary['Twait_ns'], 3)} ns`，`VC={fnum(summary['VC_mV'], 4)} mV`，面积阈值 `Λ={float(summary['Lambda']):.6e}`。
...
| 数据集 | 点数 | 文件 |
|---|---:|---|
| common-mode 密集扫频 | {summary['common_points']} | `iqcot_four_phase_dense_common_sweep.csv` |
| common-mode 幅值线性 | {summary['amplitude_linearity_points']} | `iqcot_four_phase_common_amplitude_linearity.csv` |
| 执行量扫描 | {summary['actuator_sweep_points']} | `iqcot_four_phase_actuator_sweep.csv` |
| 执行量矩阵 | 9 | `iqcot_four_phase_actuator_matrix.csv` |
| DCR 失配设计网格 | {summary['mismatch_grid_points']} | `iqcot_four_phase_mismatch_grid.csv` |
| 数字 jitter 预算 | 80+ | `iqcot_digital_*_budget.csv` |

## 6. 结果一：He-only 模型的频率选择性失效

旧稿只给出少数 common-mode 频点，容易被质疑为挑点。本文新增 {summary['common_points']} 个频点，覆盖 `ω/π=0.002` 到 `0.85`。结果显示，He-only 模型并非处处错误：在 `ω/π={float(summary['best_common_ratio']):.3g}` 附近，误差仅为 {fnum(summary['best_common_he_error_pct'], 2)}%。但在 `ω/π={float(summary['worst_common_ratio']):.3g}` 附近，相对误差达到 {fnum(summary['worst_common_he_error_pct'], 2)}%。这个百分比应谨慎解读：该频点真实 wait 响应幅值接近局部谷值，因此相对误差会被放大；更可靠的读法是同时观察 Fig. 7 左侧的绝对幅值曲线和右侧的误差曲线。结论不是“He-only 永远错误”，而是它无法表达 `K(z)` 的频率选择性。
...
|---|---:|---:|---:|---|
| Lambda_diff | 1e-13 | {fnum(lam_m2['G_current_pkpk_mA'], 5)} mA | {fnum(lam_m2['G_phase_wait_pkpk_ns'], 4)} ns | 基本不改变 DC 均流，主要移动事件间隔 |
| Ton_diff | 0.1 ns | {fnum(ton_m2['G_current_pkpk_mA'], 2)} mA | {fnum(ton_m2['G_phase_wait_pkpk_ns'], 2)} ns | 强均流执行量，但相位代价大 |
| delay_diff | 0.1 ns | {fnum(delay_m2['G_current_pkpk_mA'], 5)} mA | {fnum(delay_m2['G_phase_wait_pkpk_ns'], 4)} ns | 主要是检测时序/phase-spacing 扰动 |
...
本文采用 `2Λ` 作为面积阈值 full-scale 的保守预算假设。12 bit 面积阈值对应 `q_Λ/Λ≈{fnum(bit12['q_lambda_over_nominal'], 6)}`，估计 common wait jitter 为 {fnum(bit12['sigma_wait_common_rms_ns'], 4)} ns rms，m2 phase-spacing jitter 为 {fnum(bit12['sigma_phase_spacing_m2_ns'], 4)} ns。相比之下，1 ns 检测时钟的单事件延迟 rms 为 {fnum(clk1['sigma_single_event_delay_ns'], 4)} ns，10 ps Ton 分辨率对应 m2 电流量化尺度约 {fnum(ton10['sigma_current_pkpk_m2_mA'], 2)} mA，phase-spacing 尺度约 {fnum(ton10['sigma_phase_spacing_m2_ns'], 3)} ns。综合 12 bit、1 ns 检测时钟和 10 ps Ton 分辨率时，估计 common wait jitter 为 {fnum(comb_pick['sigma_common_wait_ns_rss'], 4)} ns rms，phase-spacing jitter 为 {fnum(comb_pick['sigma_phase_spacing_ns_rss'], 4)} ns rms，均流量化尺度为 {fnum(comb_pick['sigma_current_mA_rss'], 2)} mA。
...
| 论断 | 证据 | 状态 |
|---|---|---|
| IEK 必须包含动态事件核 `K(z)`，He-only 是退化近似 | 单相动态 IEK 验证误差 <0.00018%；四相密集扫频中 He-only 最坏误差 {fnum(summary['worst_common_he_error_pct'], 2)}% | 已由本地脚本支持 |
| `Λ_diff` 不是主要 DC 均流执行量 | m2 模式下 `G_IΛ≈{fnum(lam_m2['G_current_pkpk_mA'], 5)} mA/(1e-13)` | 已由执行量矩阵支持 |
| `Ton_diff` 是强均流执行量，但相位代价大 | m2 模式下 `G_IT≈{fnum(ton_m2['G_current_pkpk_mA'], 2)} mA/(0.1 ns)`，`G_φT≈{fnum(ton_m2['G_phase_wait_pkpk_ns'], 2)} ns/(0.1 ns)` | 已由执行量矩阵支持 |
| 检测延迟差模主要是时序扰动 | m2 模式下 `G_φd≈{fnum(delay_m2['G_phase_wait_pkpk_ns'], 4)} ns/(0.1 ns)`，电流增益很小 | 已由执行量矩阵支持 |
...
"""
    replacements = {
        "{summary['common_points']}": summary["common_points"],
        "{summary['amplitude_linearity_points']}": summary["amplitude_linearity_points"],
        "{summary['actuator_sweep_points']}": summary["actuator_sweep_points"],
        "{summary['mismatch_grid_points']}": summary["mismatch_grid_points"],
        "{summary['Vin']}": summary["Vin"],
        "{summary['Vref']}": summary["Vref"],
        "{summary['Iout']}": summary["Iout"],
        "{summary['Iph']}": summary["Iph"],
        "{float(summary['L'])*1e9:.0f}": f"{float(summary['L'])*1e9:.0f}",
        "{float(summary['C'])*1e3:.1f}": f"{float(summary['C'])*1e3:.1f}",
        "{float(summary['DCR'])*1e3:.1f}": f"{float(summary['DCR'])*1e3:.1f}",
        "{float(summary['Ri'])*1e3:.1f}": f"{float(summary['Ri'])*1e3:.1f}",
        "{fnum(summary['Ton_ns'], 3)}": fnum(summary["Ton_ns"], 3),
        "{fnum(summary['Twait_ns'], 3)}": fnum(summary["Twait_ns"], 3),
        "{fnum(summary['VC_mV'], 4)}": fnum(summary["VC_mV"], 4),
        "{float(summary['Lambda']):.6e}": f"{float(summary['Lambda']):.6e}",
        "{fnum(summary['worst_common_he_error_pct'], 2)}": fnum(summary["worst_common_he_error_pct"], 2),
        "{float(summary['best_common_ratio']):.3g}": f"{float(summary['best_common_ratio']):.3g}",
        "{fnum(summary['best_common_he_error_pct'], 2)}": fnum(summary["best_common_he_error_pct"], 2),
        "{float(summary['worst_common_ratio']):.3g}": f"{float(summary['worst_common_ratio']):.3g}",
        "{fnum(lam_m2['G_current_pkpk_mA'], 4)}": fnum(lam_m2["G_current_pkpk_mA"], 4),
        "{fnum(lam_m2['G_current_pkpk_mA'], 5)}": fnum(lam_m2["G_current_pkpk_mA"], 5),
        "{fnum(lam_m2['G_phase_wait_pkpk_ns'], 4)}": fnum(lam_m2["G_phase_wait_pkpk_ns"], 4),
        "{fnum(ton_m2['G_current_pkpk_mA'], 2)}": fnum(ton_m2["G_current_pkpk_mA"], 2),
        "{fnum(ton_m2['G_phase_wait_pkpk_ns'], 2)}": fnum(ton_m2["G_phase_wait_pkpk_ns"], 2),
        "{fnum(delay_m2['G_current_pkpk_mA'], 5)}": fnum(delay_m2["G_current_pkpk_mA"], 5),
        "{fnum(delay_m2['G_phase_wait_pkpk_ns'], 4)}": fnum(delay_m2["G_phase_wait_pkpk_ns"], 4),
        "{fnum(bit12['q_lambda_over_nominal'], 6)}": fnum(bit12["q_lambda_over_nominal"], 6),
        "{fnum(bit12['sigma_wait_common_rms_ns'], 4)}": fnum(bit12["sigma_wait_common_rms_ns"], 4),
        "{fnum(bit12['sigma_phase_spacing_m2_ns'], 4)}": fnum(bit12["sigma_phase_spacing_m2_ns"], 4),
        "{fnum(clk1['sigma_single_event_delay_ns'], 4)}": fnum(clk1["sigma_single_event_delay_ns"], 4),
        "{fnum(ton10['sigma_current_pkpk_m2_mA'], 2)}": fnum(ton10["sigma_current_pkpk_m2_mA"], 2),
        "{fnum(ton10['sigma_phase_spacing_m2_ns'], 3)}": fnum(ton10["sigma_phase_spacing_m2_ns"], 3),
        "{fnum(comb_pick['sigma_common_wait_ns_rss'], 4)}": fnum(comb_pick["sigma_common_wait_ns_rss"], 4),
        "{fnum(comb_pick['sigma_phase_spacing_ns_rss'], 4)}": fnum(comb_pick["sigma_phase_spacing_ns_rss"], 4),
        "{fnum(comb_pick['sigma_current_mA_rss'], 2)}": fnum(comb_pick["sigma_current_mA_rss"], 2),
    }
    for old, new in replacements.items():
        md = md.replace(old, str(new))
    return md


def markdown_to_html(md, path):
    body = markdown.markdown(md, extensions=["tables", "fenced_code"])
    html = f"""<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
<title>四相数字 IQCOT IEK 论文 v2</title>
<style>
...
def add_markdown_table(doc, lines, start):
    rows = []
    i = start
...
def main():
    md = make_markdown()
    md_path = OUT / "iqcot_multiphase_iek_paper_v2_four_phase_dense.md"
    html_path = OUT / "iqcot_multiphase_iek_paper_v2_four_phase_dense.html"
    docx_path = OUT / "iqcot_multiphase_iek_article_v2_four_phase_dense.docx"
    md_path.write_text(md, encoding="utf-8")
    markdown_to_html(md, html_path)
    markdown_to_docx(md, docx_path)
    print("Exported article v2")
    print(f"MD={md_path}")
