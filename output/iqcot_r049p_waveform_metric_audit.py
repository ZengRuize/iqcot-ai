"""R049P waveform audit for the 1.600us release-midpoint test."""
...
RESULTS = OUT / "r049p_release_midpoint_results_full.csv"
...
CASES = [
    ("0.050", "A0", "baseline", "r049p_20A_off0p050_a0_r049p_release_midpoint_wave.csv"),
    ("0.050", "A2_1p600", "release_1p600us", "r049p_20A_off0p050_a2_rel1p600_r049p_release_midpoint_wave.csv"),
    ("0.105", "A0", "baseline", "r049p_20A_off0p105_a0_r049p_release_midpoint_wave.csv"),
    ("0.105", "A2_1p600", "release_1p600us", "r049p_20A_off0p105_a2_rel1p600_r049p_release_midpoint_wave.csv"),
]
...
def pair_rows(rows):
    ...
    for offset in ["0.050", "0.105"]:
        for window, _, _ in WINDOWS:
            ...
            a2 = by_key.get((offset, window, "A2_1p600"))
...
def write_report(pair_deltas, baseline_failures, dec, path):
    ...
    lines += [
        ...
        f"```text\n{dec}\n```\n",
        "\n## Interpretation\n",
        "`1.600us` is the first narrow midpoint after R049O. It should be interpreted "
        "only relative to the R049O/R049N timing bracket: `1.250-1.450us` was "
        "transparent, while `1.685us` was active but too hard.\n",
    ]
...
def main():
    ...
    case_path = OUT / "r049p_waveform_metric_case_windows.csv"
    pair_path = OUT / "r049p_waveform_metric_pair_delta.csv"
    report_path = OUT / "r049p_waveform_metric_summary.md"
    write_csv(case_path, case_rows)
    write_csv(pair_path, pair_deltas)
    write_report(pair_deltas, baseline_failures, dec, report_path)
    print(f"R049P_CASE_WINDOWS={case_path}")
    print(f"R049P_PAIR_DELTA={pair_path}")
    print(f"R049P_REPORT={report_path}")
    print(f"R049P_DECISION={dec}")
