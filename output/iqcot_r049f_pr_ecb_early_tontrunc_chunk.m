function rows = iqcot_r049f_pr_ecb_early_tontrunc_chunk(runSimulink, maxCases, startRow)
%IQCOT_R049F_PR_ECB_EARLY_TONTRUNC_CHUNK Run the early-trigger diagnostic.
%
% Dry run:
%   iqcot_r049f_pr_ecb_early_tontrunc_chunk(false)
%
% True run:
%   iqcot_r049f_pr_ecb_early_tontrunc_chunk(true)
%
% Scope: same R049E 40A->20A two-offset chunk, but with a load-step-synchronous
% early Ton-truncation window instead of the over-voltage-triggered window.
...
setappdata(0, "iqcot_r049f_run_simulink", runSimulink);
setappdata(0, "iqcot_r049f_max_cases", maxCases);
setappdata(0, "iqcot_r049f_start_row", startRow);

run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");

runSimulink = getappdata(0, "iqcot_r049f_run_simulink");
maxCases = getappdata(0, "iqcot_r049f_max_cases");
startRow = getappdata(0, "iqcot_r049f_start_row");
rmappdata(0, "iqcot_r049f_run_simulink");
rmappdata(0, "iqcot_r049f_max_cases");
rmappdata(0, "iqcot_r049f_start_row");
...
modelFile = iqcot_r049f_build_early_tontrunc_model();
[~, model] = fileparts(modelFile);

fullPlan = buildPlan();
planPath = fullfile(chunkRoot, "r049f_early_tontrunc_plan.csv");
...
if ~runSimulink
    ...
    fprintf("R049F_EARLY_TONTRUNC_PLAN=%s\n", planPath);
    fprintf("Dry run only. To run the trigger-timing chunk:\n");
    fprintf("  iqcot_r049f_pr_ecb_early_tontrunc_chunk(true)\n");
    return;
end
...
for k = 1:height(plan)
    ...
    fprintf("R049F case %s: %s, %.1f A -> %.3f A, offset %.3f us, early window %.3f us\n", ...
    ...
    try
        ...
        warning("iqcot:R049FEarlyTonTruncCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(chunkRoot, "r049f_early_tontrunc_results_" + runLabel + ".csv");
comparisonPath = fullfile(chunkRoot, "r049f_early_tontrunc_comparison_" + runLabel + ".csv");
reportPath = fullfile(chunkRoot, "r049f_early_tontrunc_report_" + runLabel + ".md");
...
fprintf("R049F_EARLY_TONTRUNC_RESULTS=%s\n", resultPath);
fprintf("R049F_EARLY_TONTRUNC_COMPARISON=%s\n", comparisonPath);
fprintf("R049F_EARLY_TONTRUNC_REPORT=%s\n", reportPath);
fprintf("R049F_DECISION=%s\n", decision);
end

function plan = buildPlan()
case_id = [
    "r049f_20A_off0p050_a0";
    "r049f_20A_off0p050_a2_early";
    "r049f_20A_off0p105_a0";
    "r049f_20A_off0p105_a2_early"
];
controller = [
    "A0_no_trunc";
    "A2_early_ton_trunc";
...
objective = repmat("r049f_trigger_timing_diagnostic", numel(case_id), 1);
role = [
    "baseline_same_model";
    "early_trigger_ton_truncation";
    "baseline_same_model";
    "early_trigger_ton_truncation"
];
...
truncMask = truncGlobal(:) > 0.5;
if any(truncMask)
    firstTruncIdx = find(truncMask, 1, "first");
    lastTruncIdx = find(truncMask, 1, "last");
    firstTruncUs = 1e6 * tRel(firstTruncIdx);
    lastTruncUs = 1e6 * tRel(lastTruncIdx);
    qh4AtFirstTrunc = qh(firstTruncIdx, 4);
else
    firstTruncUs = NaN;
    lastTruncUs = NaN;
    qh4AtFirstTrunc = NaN;
...
wavePath = fullfile(dataRoot, string(spec.case_id) + "_r049f_early_tontrunc_wave.csv");
...
row = table(true, "", string(planRow.case_id), string(planRow.controller), ...
    ...
    firstTruncUs, lastTruncUs, qh4AtFirstTrunc, ...
...
names = {'success','error_message','case_id','controller','target_label','objective','role', ...
    ...
    'first_trunc_us','last_trunc_us','qh4_at_first_trunc', ...
...
for k = 1:numel(offsets)
    ...
    a2 = sub(sub.controller == "A2_early_ton_trunc", :);
    ...
    row = table(offset, a0.delta_v_actual_peak_mV, a2.delta_v_actual_peak_mV, ...
        ...
        a0.remaining_ton4_ns - a2.remaining_ton4_ns, ...
        a2.trunc_duration_us, a2.trunc_edge_count, a2.first_trunc_us, ...
        a2.last_trunc_us, a2.qh4_at_first_trunc, ...
        ...
        'a2_trunc_duration_us','a2_trunc_edge_count','a2_first_trunc_us','a2_last_trunc_us', ...
        'a2_qh4_at_first_trunc','a2_total_ton_min_hits'});
...
if isempty(activeRow) || activeRow.a2_trunc_duration_us <= 0
    decision = "IMPLEMENTATION_ISSUE";
    return;
end
...
reportPath = fullfile(chunkRoot, "r049f_early_tontrunc_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049F Early Ton-Truncation Trigger-Timing Dry Run\n\n");
fprintf(fid, "Prepared 40A->20A crossed with two phase offsets.\n\n");
fprintf(fid, "- Derived model: %s\n", modelFile);
fprintf(fid, "- Plan: %s\n", planPath);
fprintf(fid, "- The completed R049E `.slx` model is copied, not modified.\n");
fprintf(fid, "- R049C_TonTrunc_Global is reconfigured to a two-input time-window AND.\n\n");
fprintf(fid, "Run with `iqcot_r049f_pr_ecb_early_tontrunc_chunk(true)`.\n");
end

function writeRunReport(rows, comparison, decision, resultPath, comparisonPath, reportPath, modelFile, runLabel)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049F PR-ECB Early Ton-Truncation Trigger-Timing Chunk\n\n");
fprintf(fid, "Run label: `%s`\n\n", runLabel);
fprintf(fid, "## Scope and hypothesis\n\n");
fprintf(fid, "- Model version: R049F copy of R049E with Ton-truncation flag changed to load-step-synchronous time-window logic.\n");
fprintf(fid, "- Diagnostic: `40A -> 20A` at offsets `0.05us` and `0.105us`.\n");
fprintf(fid, "- A2 early trigger: `Tton_trunc_min=5ns`, `Tton_trunc_window=80ns`.\n");
...
fprintf(fid, "- Wave snapshots: `output/data/*_r049f_early_tontrunc_wave.csv`\n\n");
fprintf(fid, "## Per-case results\n\n");
fprintf(fid, "| case | ctrl | offset us | peak mV | t_peak us | rem Ton4 ns | trunc us | first trunc us | qh4 first trunc | undershoot mV | final mV |\n");
fprintf(fid, "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n");
for k = 1:height(rows)
    if rows.success(k)
        fprintf(fid, "| %s | %s | %.3f | %.4f | %.4f | %.4f | %.4f | %.4f | %.0f | %.4f | %.4f |\n", ...
            rows.case_id(k), rows.controller(k), rows.load_step_offset_us(k), ...
            rows.delta_v_actual_peak_mV(k), rows.t_peak_actual_us(k), ...
            rows.remaining_ton4_ns(k), rows.trunc_duration_us(k), ...
            rows.first_trunc_us(k), rows.qh4_at_first_trunc(k), ...
...
if isempty(comparison)
    ...
    fprintf(fid, "| offset us | A0 peak mV | A2 peak mV | improvement mV | rem Ton4 reduction ns | A2 first trunc us | A2 qh4 first trunc | A2-A0 undershoot mV |\n");
    fprintf(fid, "|---:|---:|---:|---:|---:|---:|---:|---:|\n");
    for k = 1:height(comparison)
        fprintf(fid, "| %.3f | %.4f | %.4f | %.4f | %.4f | %.4f | %.0f | %.4f |\n", ...
            comparison.load_step_offset_us(k), comparison.a0_peak_mV(k), ...
            comparison.a2_peak_mV(k), comparison.peak_improvement_mV(k), ...
            comparison.remaining_ton4_reduction_ns(k), comparison.a2_first_trunc_us(k), ...
            comparison.a2_qh4_at_first_trunc(k), comparison.a2_minus_a0_undershoot_mV(k));
    end
end
fprintf(fid, "\n## Decision\n\n");
fprintf(fid, "```text\n%s\n```\n\n", decision);
fprintf(fid, "This decision applies only to this R049F trigger-timing diagnostic chunk.\n");
end
