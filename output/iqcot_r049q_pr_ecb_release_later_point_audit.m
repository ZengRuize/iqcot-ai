function rows = iqcot_r049q_pr_ecb_release_later_point_audit(runSimulink)
%IQCOT_R049Q_PR_ECB_RELEASE_LATER_POINT_AUDIT Test one later binary release.
%
% R049Q keeps the R049N upstream-causal release interface and tests exactly
% one slightly later binary release delay after the R049P midpoint:
%
%   Tphase_release_delay = 1.630 us
%
% This is intentionally narrower than R049O.  It brackets between the
% transparent 1.450us release and the active-but-hard 1.685us release.

clc;

if nargin < 1 || isempty(runSimulink), runSimulink = false; end
setappdata(0, "iqcot_r049q_run_simulink", runSimulink);
run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");
runSimulink = getappdata(0, "iqcot_r049q_run_simulink");
rmappdata(0, "iqcot_r049q_run_simulink");
...
plan = buildPlan();
planPath = fullfile(chunkRoot, "r049q_release_later_point_plan.csv");
writetable(plan, planPath);

if ~runSimulink
    rows = plan;
    writeDryRunReport(chunkRoot, planPath, modelFile);
    fprintf("R049Q_RELEASE_LATER_POINT_PLAN=%s\n", planPath);
    fprintf("Dry run only. Run with iqcot_r049q_pr_ecb_release_later_point_audit(true).\n");
    return;
end
...
for k = 1:height(plan)
    ...
    fprintf("R049Q case %s: %s, offset %.3fus, release %.3fus\n", ...
    ...
    try
        ...
        warning("iqcot:R049QReleaseLaterPointCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(chunkRoot, "r049q_release_later_point_results_full.csv");
reportPath = fullfile(chunkRoot, "r049q_release_later_point_report_full.md");
writetable(rows, resultPath);
writeRunReport(rows, resultPath, reportPath, modelFile);
fprintf("R049Q_RELEASE_LATER_POINT_RESULTS=%s\n", resultPath);
fprintf("R049Q_RELEASE_LATER_POINT_REPORT=%s\n", reportPath);
end

function plan = buildPlan()
case_id = [
    "r049q_20A_off0p050_a0";
    "r049q_20A_off0p050_a2_rel1p630";
    "r049q_20A_off0p105_a0";
    "r049q_20A_off0p105_a2_rel1p630"
];
controller = [
    "A0_no_inhibit";
    "A2_release_1p630us";
...
wavePath = fullfile(dataRoot, string(spec.case_id) + "_r049q_release_later_point_wave.csv");
...
reportPath = fullfile(chunkRoot, "r049q_release_later_point_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049Q Release-Later-Point Dry Run\n\n");
fprintf(fid, "- Model: `%s`\n", modelFile);
fprintf(fid, "- Plan: `%s`\n", planPath);
fprintf(fid, "- Release delay: `1.630 us`.\n");
end

function writeRunReport(rows, resultPath, reportPath, modelFile)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049Q PR-ECB Release-Later-Point Audit\n\n");
fprintf(fid, "- Model: `%s`\n", modelFile);
