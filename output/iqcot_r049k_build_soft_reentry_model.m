function modelFile = iqcot_r049k_build_soft_reentry_model()
%IQCOT_R049K_BUILD_SOFT_REENTRY_MODEL Build shortened soft-reentry copy.
%
% R049K copies the completed R049I model into a new derived `.slx` file and
% inserts one request-path action:
%
%   allow_to_scheduler = existing_allow AND NOT(soft_reentry)
%
% where soft_reentry is a load-step-relative window.  The runner selects
...
dstModelName = "four_phase_iek_pr_ecb_control_r049k_soft_reentry";
...
insertSoftReentryGate(dstModelName);
...
fprintf("R049K_SOFT_REENTRY_MODEL=%s\n", modelFile);
end

function insertSoftReentryGate(modelName)
...
clockPath = modelName + "/R049K_Clock";
startConstPath = modelName + "/R049K_Inhibit_Start";
endConstPath = modelName + "/R049K_Inhibit_End";
afterPath = modelName + "/R049K_After_Inhibit_Start";
beforePath = modelName + "/R049K_Before_Inhibit_End";
inhibitPath = modelName + "/R049K_Soft_Reentry";
notInhibitPath = modelName + "/R049K_Not_Soft_Reentry";
...
add_line(modelName, "R049K_Clock/1", "R049K_After_Inhibit_Start/1", "autorouting", "on");
add_line(modelName, "R049K_Inhibit_Start/1", "R049K_After_Inhibit_Start/2", "autorouting", "on");
add_line(modelName, "R049K_Clock/1", "R049K_Before_Inhibit_End/1", "autorouting", "on");
add_line(modelName, "R049K_Inhibit_End/1", "R049K_Before_Inhibit_End/2", "autorouting", "on");
add_line(modelName, "R049K_After_Inhibit_Start/1", "R049K_Soft_Reentry/1", "autorouting", "on");
add_line(modelName, "R049K_Before_Inhibit_End/1", "R049K_Soft_Reentry/2", "autorouting", "on");
add_line(modelName, "R049K_Soft_Reentry/1", "R049K_Not_Soft_Reentry/1", "autorouting", "on");
add_line(modelName, "R049K_Not_Soft_Reentry/1", getBlockName(logicPath) + "/3", "autorouting", "on");

addLoggedBoolFanout(modelName, "R049K_Soft_Reentry", "soft_reentry", "soft_reentry", 1810, 300);
addLoggedBoolFanout(modelName, "R049K_Soft_Reentry", "protect_state", "protect_state", 1810, 345);
for phase = 1:4
    y = 410 + (phase - 1) * 40;
    addLoggedBoolFanout(modelName, "R049K_Soft_Reentry", ...
        "pulse_inhibit" + phase, "pulse_inhibit" + phase, 1810, y);
end
markBlockOutport(notInhibitPath, 1, "not_soft_reentry");
end
...
convPath = modelName + "/R049K_" + blockStem + "_double";
termPath = modelName + "/R049K_" + blockStem + "_Term";
