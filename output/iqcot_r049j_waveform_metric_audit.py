"""R049J offline waveform-metric audit for deferred post-active inhibit.

This script does not run Simulink.  It audits the R049J wave CSV files emitted
by ``iqcot_r049j_pr_ecb_post_active_inhibit_chunk(true)`` using the R049H
...
CASES = [
    CaseSpec(0.050, "A0", "baseline_same_model", "r049j_20A_off0p050_a0_r049j_post_active_inhibit_wave.csv"),
    CaseSpec(0.050, "A2", "deferred_post_active_inhibit", "r049j_20A_off0p050_a2_post_active_inhibit_r049j_post_active_inhibit_wave.csv"),
    CaseSpec(0.105, "A0", "baseline_same_model", "r049j_20A_off0p105_a0_r049j_post_active_inhibit_wave.csv"),
    CaseSpec(0.105, "A2", "deferred_post_active_inhibit", "r049j_20A_off0p105_a2_post_active_inhibit_r049j_post_active_inhibit_wave.csv"),
]
...
def audit_case(spec: CaseSpec) -> list[dict[str, object]]:
    ...
    with path.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            inhibit = float(row.get("post_active_inhibit", "0")) > 0.5
...
def write_report(pair_deltas: list[dict[str, object]], decision: str, path: Path) -> None:
    lines = [
        ...
        "Date: 2026-06-25\n",
        "## Scope\n",
        "R049J audits one new derived-Simulink request-path action chunk: "
        "`40A -> 20A` at `0.05us` and `0.105us`, A0 same-model no-inhibit "
        "versus A2 deferred post-active pulse inhibit.  Ton truncation is "
    ...
    if decision == "MODEL_REVISED":
        lines.append(
            ...
            "intent, but the `0.07-2.00us` inhibit window is too aggressive for "
            "reentry: it creates an undershoot penalty even while reducing "
            "positive recovery peaks.  The state "
            "machine should either shorten/phase-select the post-active inhibit "
...
def main() -> None:
    ...
    case_path = OUT / "r049j_waveform_metric_case_windows.csv"
    pair_path = OUT / "r049j_waveform_metric_pair_delta.csv"
    report_path = OUT / "r049j_waveform_metric_summary.md"
    write_csv(case_path, case_rows)
    write_csv(pair_path, pair_deltas)
    write_report(pair_deltas, decision, report_path)
    print(f"R049J_CASE_WINDOWS={case_path}")
    print(f"R049J_PAIR_DELTA={pair_path}")
    print(f"R049J_REPORT={report_path}")
    print(f"R049J_DECISION={decision}")
