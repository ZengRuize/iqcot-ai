function rows = iqcot_r049d_pr_ecb_tontrunc_holdout_chunk(runSimulink, maxCases, startRow)
%IQCOT_R049D_PR_ECB_TONTRUNC_HOLDOUT_CHUNK Run the R049D hold-out chunk.
%
% Dry run:
%   iqcot_r049d_pr_ecb_tontrunc_holdout_chunk(false)
%
% True run:
%   iqcot_r049d_pr_ecb_tontrunc_holdout_chunk(true)
%
% Scope: one hold-out load-drop magnitude (40A -> 10A) crossed with two phase
% offsets (0.05us active-HS boundary and 0.105us post-turnoff reference).  For
% each offset, run A0 same-model no-trunc and A2 command-path Ton truncation.
...
setappdata(0, "iqcot_r049d_run_simulink", runSimulink);
setappdata(0, "iqcot_r049d_max_cases", maxCases);
setappdata(0, "iqcot_r049d_start_row", startRow);

run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");

runSimulink = getappdata(0, "iqcot_r049d_run_simulink");
maxCases = getappdata(0, "iqcot_r049d_max_cases");
startRow = getappdata(0, "iqcot_r049d_start_row");
rmappdata(0, "iqcot_r049d_run_simulink");
rmappdata(0, "iqcot_r049d_max_cases");
rmappdata(0, "iqcot_r049d_start_row");
...
modelFile = iqcot_r049d_build_tontrunc_holdout_model();
[~, model] = fileparts(modelFile);

fullPlan = buildR049DPlan();
planPath = fullfile(chunkRoot, "r049d_tontrunc_holdout_plan.csv");
...
if ~runSimulink
    ...
    fprintf("R049D_TONTRUNC_HOLDOUT_PLAN=%s\n", planPath);
    fprintf("Dry run only. To run the hold-out chunk:\n");
    fprintf("  iqcot_r049d_pr_ecb_tontrunc_holdout_chunk(true)\n");
    return;
end
...
rows = table();
for k = 1:height(plan)
    spec = makeR049DSpec(plan(k, :), common);
    fprintf("R049D case %s: %s, %.1f A -> %.3f A, offset %.3f us, threshold %.3f mV\n", ...
    ...
    try
        ...
        warning("iqcot:R049DTonTruncHoldoutCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(chunkRoot, "r049d_tontrunc_holdout_results_" + runLabel + ".csv");
comparisonPath = fullfile(chunkRoot, "r049d_tontrunc_holdout_comparison_" + runLabel + ".csv");
reportPath = fullfile(chunkRoot, "r049d_tontrunc_holdout_report_" + runLabel + ".md");
...
fprintf("R049D_TONTRUNC_HOLDOUT_RESULTS=%s\n", resultPath);
fprintf("R049D_TONTRUNC_HOLDOUT_COMPARISON=%s\n", comparisonPath);
fprintf("R049D_TONTRUNC_HOLDOUT_REPORT=%s\n", reportPath);
fprintf("R049D_DECISION=%s\n", decision);
end

function plan = buildR049DPlan()
case_id = [
    "r049d_10A_off0p050_a0";
    "r049d_10A_off0p050_a2_tontrunc";
    "r049d_10A_off0p105_a0";
    "r049d_10A_off0p105_a2_tontrunc"
];
controller = [
    "A0_no_trunc";
    "A2_ton_trunc";
...
target_label = repmat("10A", numel(case_id), 1);
base_load_A = 40 * ones(numel(case_id), 1);
target_load_A = 10 * ones(numel(case_id), 1);
...
objective = repmat("r049d_tontrunc_holdout", numel(case_id), 1);
role = [
    "baseline_same_model";
    "holdout_ton_truncation";
    "baseline_same_model";
    "holdout_ton_truncation"
];
...
function spec = makeR049DSpec(planRow, common)
spec = struct();
...
wavePath = fullfile(dataRoot, string(spec.case_id) + "_r049d_tontrunc_holdout_wave.csv");
...
if activeRow.a2_trunc_duration_us <= 0
    if activeRow.a0_peak_mV >= 2.0
        decision = "IMPLEMENTATION_ISSUE";
    else
...
reportPath = fullfile(chunkRoot, "r049d_tontrunc_holdout_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049D Ton-Truncation Hold-Out Dry Run\n\n");
fprintf(fid, "Prepared 40A->10A crossed with two phase offsets.\n\n");
fprintf(fid, "- Derived model: %s\n", modelFile);
fprintf(fid, "- Plan: %s\n", planPath);
fprintf(fid, "- The completed R049C `.slx` model is copied, not modified.\n\n");
fprintf(fid, "Run with `iqcot_r049d_pr_ecb_tontrunc_holdout_chunk(true)`.\n");
end

function writeRunReport(rows, comparison, decision, resultPath, comparisonPath, reportPath, modelFile, runLabel)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049D PR-ECB Ton-Truncation Hold-Out Chunk\n\n");
fprintf(fid, "Run label: `%s`\n\n", runLabel);
fprintf(fid, "## Scope and hypothesis\n\n");
fprintf(fid, "- Model version: R049D copy of the R049C command-path Ton-truncation model.\n");
fprintf(fid, "- Hold-out: `40A -> 10A` at offsets `0.05us` and `0.105us`.\n");
fprintf(fid, "- Hypothesis: the active-HS offset should benefit from Ton truncation without worsening the post-turnoff offset or secondary response.\n");
...
fprintf(fid, "- Wave snapshots: `output/data/*_r049d_tontrunc_holdout_wave.csv`\n\n");
...
fprintf(fid, "\n## Decision\n\n");
fprintf(fid, "```text\n%s\n```\n\n", decision);
fprintf(fid, "This decision applies only to this R049D hold-out chunk.\n");
end
