"""R049R waveform audit for the 1.615us release-between-point test."""
...
RESULTS = OUT / "r049r_release_between_point_results_full.csv"
...
CASES = [
    ("0.050", "A0", "baseline", "r049r_20A_off0p050_a0_r049r_release_between_point_wave.csv"),
    ("0.050", "A2_1p615", "release_1p615us", "r049r_20A_off0p050_a2_rel1p615_r049r_release_between_point_wave.csv"),
    ("0.105", "A0", "baseline", "r049r_20A_off0p105_a0_r049r_release_between_point_wave.csv"),
    ("0.105", "A2_1p615", "release_1p615us", "r049r_20A_off0p105_a2_rel1p615_r049r_release_between_point_wave.csv"),
]
...
def audit_case(offset, controller, action, filename):
    ...
    for name, lo, hi in WINDOWS:
        ...
        out.append({
            ...
            "inhibit_raw_duration_us": duration_true(raw, "inhibit_raw"),
            "wave_csv": str(path),
        })
    return out
...
def pair_rows(rows):
    ...
    for offset in ["0.050", "0.105"]:
        for window, _, _ in WINDOWS:
            ...
            a2 = by_key.get((offset, window, "A2_1p615"))
...
def decision(pair_deltas, baseline_failures):
    ...
    recovery = [r for r in pair_deltas if r["window"] == "recovery_peak"]
    has_effect = any(abs(float(r["peak_improvement_mV"])) >= 0.02 for r in pair_deltas)
    undershoot_bad = any(float(r["undershoot_improvement_mV"]) < -0.2 for r in recovery)
    if not has_effect:
...
def write_report(pair_deltas, baseline_failures, dec, path):
    ...
    lines += [
        ...
        f"```text\n{dec}\n```\n",
        "\n## Interpretation\n",
        "`1.615us` is the first between-point after R049P's `1.600us` midpoint "
        "and R049Q's `1.630us` later point. It should be interpreted only "
        "relative to the R049O/R049P/R049Q/R049N timing bracket: "
        "`1.250-1.450us` was transparent, `1.600us` was offset-selective, "
        "`1.630us` had stronger undershoot penalty, and `1.685us` was active "
        "but too hard.\n",
    ]
...
def main():
    ...
    case_path = OUT / "r049r_waveform_metric_case_windows.csv"
    pair_path = OUT / "r049r_waveform_metric_pair_delta.csv"
    report_path = OUT / "r049r_waveform_metric_summary.md"
    write_csv(case_path, case_rows)
    write_csv(pair_path, pair_deltas)
    write_report(pair_deltas, baseline_failures, dec, report_path)
    print(f"R049R_CASE_WINDOWS={case_path}")
    print(f"R049R_PAIR_DELTA={pair_path}")
    print(f"R049R_REPORT={report_path}")
    print(f"R049R_DECISION={dec}")
