function rows = iqcot_r049o_pr_ecb_release_timing_micro_audit(runSimulink)
%IQCOT_R049O_PR_ECB_RELEASE_TIMING_MICRO_AUDIT Run two-delay release timing audit.
%
% R049O reuses the R049N upstream-causal independent release-clock interface
% and tests two earlier release delays, without changing the controller
% structure:
%
%   release_clock = t_load_step + Tphase_release_delay
%   Tphase_release_delay = 1.250 us or 1.450 us
%
% The scope is intentionally tiny: two offsets, one A0 per offset, and two A2
% delays per offset.  Ton truncation remains disabled.

clc;

if nargin < 1 || isempty(runSimulink), runSimulink = false; end
setappdata(0, "iqcot_r049o_run_simulink", runSimulink);
run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");
runSimulink = getappdata(0, "iqcot_r049o_run_simulink");
rmappdata(0, "iqcot_r049o_run_simulink");
...
plan = buildPlan();
planPath = fullfile(chunkRoot, "r049o_release_timing_plan.csv");
writetable(plan, planPath);

if ~runSimulink
    rows = plan;
    writeDryRunReport(chunkRoot, planPath, modelFile);
    fprintf("R049O_RELEASE_TIMING_PLAN=%s\n", planPath);
    fprintf("Dry run only. Run with iqcot_r049o_pr_ecb_release_timing_micro_audit(true).\n");
    return;
end
...
for k = 1:height(plan)
    ...
    fprintf("R049O case %s: %s, offset %.3fus, release %.3fus\n", ...
    ...
    try
        ...
        warning("iqcot:R049OReleaseTimingCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(chunkRoot, "r049o_release_timing_results_full.csv");
reportPath = fullfile(chunkRoot, "r049o_release_timing_report_full.md");
writetable(rows, resultPath);
writeRunReport(rows, resultPath, reportPath, modelFile);
fprintf("R049O_RELEASE_TIMING_RESULTS=%s\n", resultPath);
fprintf("R049O_RELEASE_TIMING_REPORT=%s\n", reportPath);
end

function plan = buildPlan()
case_id = [
    "r049o_20A_off0p050_a0";
    "r049o_20A_off0p050_a2_rel1p250";
    "r049o_20A_off0p050_a2_rel1p450";
    "r049o_20A_off0p105_a0";
    "r049o_20A_off0p105_a2_rel1p250";
    "r049o_20A_off0p105_a2_rel1p450"
];
controller = [
    "A0_no_inhibit";
    "A2_release_1p250us";
...
post_inhibit_delay_us = 0.070 * ones(numel(case_id), 1);
post_inhibit_window_us = [-0.001; 1.690; 1.690; -0.001; 1.690; 1.690];
phase_release_delay_us = [1.250; 1.250; 1.450; 1.250; 1.250; 1.450];
objective = repmat("r049o_release_timing_micro_audit", numel(case_id), 1);
role = [
    "baseline_same_model";
    "release_timing_probe";
    "release_timing_probe";
    "baseline_same_model";
    "release_timing_probe";
    "release_timing_probe"
...
wavePath = fullfile(dataRoot, string(spec.case_id) + "_r049o_release_timing_wave.csv");
wave = table(1e6 * tRel, vout, spec.load_A * ones(numel(t), 1), ...
    il(:,1), il(:,2), il(:,3), il(:,4), ...
    qh(:,1), qh(:,2), qh(:,3), qh(:,4), req, allowCtrl, inhibitRaw, ...
    releaseClock, oneShotDone, phaseIdx, truncGlobal, ...
    ...
    'one_shot_done','phase_idx','ton_trunc_global'});
...
s.delta_v_actual_peak_mV = 1e3 * deltaVPeak;
...
s = struct("success", false, "error_message", "", ...
    ...
    "t_load_step_us", N, "vout0_V", N, ...
    "il1_0_A", N, "il2_0_A", N, "il3_0_A", N, "il4_0_A", N, ...
    "qh1_at_step", N, "qh2_at_step", N, "qh3_at_step", N, "qh4_at_step", N, ...
    "remaining_ton1_ns", N, "remaining_ton2_ns", N, "remaining_ton3_ns", N, "remaining_ton4_ns", N, ...
    "delta_v_actual_peak_mV", N, "t_peak_actual_us", N, ...
    "secondary_undershoot_mV", N, "final_error_mV", N, ...
    "global_trunc_duration_us", N, "inhibit_raw_duration_us", N, ...
    "effective_inhibit_duration_us", N, "release_clock_edge_count", N, ...
    "release_clock_time_us", N, "one_shot_edge_count", N, ...
    "one_shot_time_us", N, "req_edges_during_inhibit", N, "wave_csv", "");
end
...
reportPath = fullfile(chunkRoot, "r049o_release_timing_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049O Release-Timing Micro-Audit Dry Run\n\n");
fprintf(fid, "- Model: `%s`\n", modelFile);
fprintf(fid, "- Plan: `%s`\n", planPath);
fprintf(fid, "- Release delays: `1.250 us`, `1.450 us`.\n");
fprintf(fid, "- Ton truncation disabled.\n");
end

function writeRunReport(rows, resultPath, reportPath, modelFile)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049O PR-ECB Release-Timing Micro-Audit\n\n");
fprintf(fid, "- Model: `%s`\n", modelFile);
