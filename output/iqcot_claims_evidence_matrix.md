...
<!-- R049L_REPAIR_PR_ECB_CONTROLLED_REENTRY -->

### C56 / R049L repair: phase-boundary controlled-reentry implementation audit

| Claim | Evidence | Strength | Safe wording | Do not claim |
|---|---|---|---|---|
| R049L repair fixes the external R049L baseline and shows that downstream `qh1` is not a valid causal release trigger for request-path one-shot controlled reentry, because the inhibit gate suppresses the pulse needed to create the `qh1` edge. | `docs/R049L_SUPERVISOR_REVIEW_20260625.md`; `docs/pr_ecb_controlled_reentry_r049l_repair.md`; `refine-logs/LOCAL_AUDIT_R049L_REPAIR_PR_ECB_CONTROLLED_REENTRY_20260625.md`; `output/iqcot_r049l_repair_build_controlled_reentry_model.m`; `output/iqcot_r049l_repair_pr_ecb_controlled_reentry_chunk.m`; `output/iqcot_r049l_repair_waveform_metric_audit.py`; `output/cutload_pr_ecb_control/four_phase_iek_pr_ecb_control_r049l_repair_controlled_reentry.slx`; `output/cutload_pr_ecb_control/r049l_repair_controlled_reentry_results_full.csv`; `output/cutload_pr_ecb_control/r049l_repair_waveform_metric_summary.md`. A0 baseline matches R049K (`2.1103/2.0936 mV`, qh4 `1/0`, remaining Ton4 `50.5/0 ns`), but A2 one-shot never fired (`one_shot_edge_count=0`, `one_shot_time_us=NaN`). Decision: `IMPLEMENTATION_ISSUE`. | Medium as implementation/wiring evidence from a real derived-Simulink chunk; not performance confirmation. | "R049L repair supports revising the controlled-reentry state machine: release triggers must be upstream of the request-path gate or independently clocked; downstream gate outputs like `qh1` are effects to audit, not causes to release the gate." | "R049L repair proves controlled reentry is ineffective, confirms PR-ECB controlled reentry, proves hardware/HIL safety, validates a complete PR-ECB controller, or provides new fixed-window performance evidence beyond R049K." |

<!-- R049M_REENTRY_UPSTREAM_BOUNDARY -->

### C57 / R049M: reentry upstream boundary structural audit

| Claim | Evidence | Strength | Safe wording | Do not claim |
|---|---|---|---|---|
| R049M shows that the current scheduler is triggered by the gated `Allow` path, so existing scheduler state/output signals are not valid causal release triggers during request-path inhibition; a new independent upstream phase-clock or predicted-slot trigger is required for the next controlled-reentry test. | `docs/pr_ecb_reentry_upstream_boundary_audit_r049m.md`; `refine-logs/LOCAL_AUDIT_R049M_REENTRY_UPSTREAM_BOUNDARY_20260625.md`; `output/iqcot_r049m_reentry_boundary_audit.m`; `output/cutload_pr_ecb_control/r049m_reentry_boundary_candidates.csv`; `output/cutload_pr_ecb_control/r049m_reentry_boundary_audit_report.md`. The verified chain is `R049L_Gate_And -> Allow -> Detect Rise Positive -> tr -> PhaseScheduler_4Phase`; `phase_state` is held inside the triggered subsystem and does not evolve during inhibit. Decision: `MODEL_REVISED`. | Medium as structural model evidence; no new switching-performance validation. | "R049M supports revising the reentry interface: implement an independent upstream phase-clock / predicted-slot release trigger, then test it in the same four-row repaired chunk." | "R049M confirms controlled reentry performance, proves hardware/HIL safety, validates a complete PR-ECB controller, or proves the independent-clock design will improve the waveform before simulation." |

<!-- R049N_PR_ECB_INDEPENDENT_CLOCK_REENTRY -->

### C58 / R049N: independent-clock controlled-reentry implementation test

| Claim | Evidence | Strength | Safe wording | Do not claim |
|---|---|---|---|---|
| R049N shows that an upstream independent release-clock can replace the invalid downstream-`qh1` release trigger and fire during request inhibition, but a fixed `1.685 us` release still produces recovery undershoot penalties under the R049H three-window gate. | `docs/pr_ecb_independent_clock_reentry_r049n.md`; `refine-logs/LOCAL_AUDIT_R049N_PR_ECB_INDEPENDENT_CLOCK_REENTRY_20260625.md`; `output/iqcot_r049n_build_independent_clock_reentry_model.m`; `output/iqcot_r049n_pr_ecb_independent_clock_reentry_chunk.m`; `output/iqcot_r049n_waveform_metric_audit.py`; `output/cutload_pr_ecb_control/four_phase_iek_pr_ecb_control_r049n_independent_clock_reentry.slx`; `output/cutload_pr_ecb_control/r049n_independent_clock_reentry_results_full.csv`; `output/cutload_pr_ecb_control/r049n_waveform_metric_summary.md`. A0 baseline passed (`2.1103/2.0936 mV`, qh4 `1/0`, remaining Ton4 `50.5/0 ns`). A2 release_clock fired at `1.686/1.685 us` and one_shot_done at `1.750/1.735 us`; recovery peak improved `+0.1127/+0.1205 mV`, but recovery undershoot worsened `-0.5597/-1.4429 mV`. Decision: `MODEL_REVISED`. | Medium as derived-Simulink switching evidence over the repaired four-row chunk; implementation evidence is stronger than performance confirmation. | "R049N supports keeping the upstream-causal release interface and revising timing/soft-reentry to reduce recovery undershoot; it fixes the R049L causal trigger issue but does not confirm the fixed `1.685 us` controller." | "R049N proves controlled reentry is solved, proves hardware/HIL safety, validates a complete PR-ECB controller, or proves independent phase-clock release is globally optimal." |

<!-- R049O_PR_ECB_RELEASE_TIMING -->

### C59 / R049O: release-timing micro-audit

| Claim | Evidence | Strength | Safe wording | Do not claim |
|---|---|---|---|---|
| R049O shows that earlier binary releases at `1.250 us` and `1.450 us` are causally valid but effectively transparent: they fire one-shot release and avoid R049N's undershoot penalty only by making A2 indistinguishable from A0. | `docs/pr_ecb_release_timing_r049o.md`; `refine-logs/LOCAL_AUDIT_R049O_PR_ECB_RELEASE_TIMING_20260625.md`; `output/iqcot_r049o_pr_ecb_release_timing_micro_audit.m`; `output/iqcot_r049o_waveform_metric_audit.py`; `output/cutload_pr_ecb_control/r049o_release_timing_results_full.csv`; `output/cutload_pr_ecb_control/r049o_waveform_metric_summary.md`. A0 baseline passed; all A2 probes fired one-shot release; R049H window deltas versus A0 were all `0.0000 mV`. Decision: `CLAIM_DOWNGRADED`. | Medium-low as a small derived-Simulink timing micro-audit; useful for bracketing but not controller confirmation. | "R049O brackets the binary release timing: `1.250-1.450 us` is too early/transparent, while R049N's `1.685 us` is active but too hard. Next test should be one intermediate point or a soft restore." | "R049O proves PR-ECB timing is solved, proves hardware/HIL behavior, validates a complete controller, or proves all earlier release timings are useless globally." |

<!-- R049P_PR_ECB_RELEASE_MIDPOINT -->

### C60 / R049P: release-midpoint audit

| Claim | Evidence | Strength | Safe wording | Do not claim |
|---|---|---|---|---|
| R049P shows that a `1.600 us` binary release is offset-selective: it is still transparent at `0.050 us`, but at `0.105 us` it preserves recovery-peak benefit with a smaller undershoot penalty than R049N. | `docs/pr_ecb_release_midpoint_r049p.md`; `refine-logs/LOCAL_AUDIT_R049P_PR_ECB_RELEASE_MIDPOINT_20260625.md`; `output/iqcot_r049p_pr_ecb_release_midpoint_audit.m`; `output/iqcot_r049p_waveform_metric_audit.py`; `output/cutload_pr_ecb_control/r049p_release_midpoint_results_full.csv`; `output/cutload_pr_ecb_control/r049p_waveform_metric_summary.md`. A0 baseline passed; one-shot fired at `1.670/1.655 us`; `0.050 us` deltas were all `0.0000 mV`; `0.105 us` recovery peak improved `+0.1244 mV` with recovery undershoot penalty `-0.7873 mV`. Decision: `MODEL_REVISED`. | Medium-low as one narrow timing point in derived-Simulink; useful for direction, not confirmation. | "R049P suggests the useful binary-release region starts near `1.600 us` for the non-active-HS offset, but a single global delay is offset-selective. Continue with one later point or soft/ramped release." | "R049P proves controlled reentry is solved, proves hardware/HIL safety, validates a complete controller, or establishes a universal release delay." |

<!-- R049Q_PR_ECB_RELEASE_LATER_POINT -->

### C61 / R049Q: release-later-point audit

| Claim | Evidence | Strength | Safe wording | Do not claim |
|---|---|---|---|---|
| R049Q shows that moving the binary release later to `1.630 us` slightly strengthens the `0.105 us` recovery-peak benefit but substantially worsens the recovery-undershoot penalty versus R049P, while `0.050 us` remains transparent. | `docs/pr_ecb_release_later_point_r049q.md`; `refine-logs/LOCAL_AUDIT_R049Q_PR_ECB_RELEASE_LATER_POINT_20260625.md`; `output/iqcot_r049q_pr_ecb_release_later_point_audit.m`; `output/iqcot_r049q_waveform_metric_audit.py`; `output/cutload_pr_ecb_control/r049q_release_later_point_results_full.csv`; `output/cutload_pr_ecb_control/r049q_waveform_metric_summary.md`. A0 baseline passed; one-shot fired at `1.670/1.695 us`; `0.050 us` deltas were all `0.0000 mV`; `0.105 us` recovery peak improved `+0.1365 mV`, but recovery undershoot worsened `-1.1109 mV` and late peak degraded `-0.0666 mV`. Decision: `MODEL_REVISED`. | Medium-low as one narrow timing point in derived-Simulink; useful for bracketing the binary timing knee, not confirmation. | "R049Q narrows the binary-release timing trade-off: later than `1.600 us` increases action strength but reintroduces the hard-release undershoot mode. Next test should be between `1.600 us` and `1.630 us` or use soft/ramped release." | "R049Q proves controlled reentry is solved, proves hardware/HIL safety, validates a complete controller, or establishes that later binary release is globally better." |

<!-- R049R_PR_ECB_RELEASE_BETWEEN_POINT -->

### C62 / R049R: release-between-point event-quantization audit

| Claim | Evidence | Strength | Safe wording | Do not claim |
|---|---|---|---|---|
| R049R shows that the binary release delay is event-quantized: `1.615 us` maps to the same `1.655 us` one-shot event and same waveform metrics as R049P `1.600 us`, while R049Q `1.630 us` crosses to a later `1.695 us` event with a larger undershoot penalty. | `docs/pr_ecb_release_between_point_r049r.md`; `refine-logs/LOCAL_AUDIT_R049R_PR_ECB_RELEASE_BETWEEN_POINT_20260625.md`; `output/iqcot_r049r_pr_ecb_release_between_point_audit.m`; `output/iqcot_r049r_waveform_metric_audit.py`; `output/cutload_pr_ecb_control/r049r_release_between_point_results_full.csv`; `output/cutload_pr_ecb_control/r049r_waveform_metric_summary.md`. A0 baseline passed; one-shot fired at `1.670/1.655 us`; `0.050 us` deltas were all `0.0000 mV`; `0.105 us` recovery peak improved `+0.1244 mV` with recovery undershoot penalty `-0.7873 mV`, matching R049P. Decision: `MODEL_REVISED`. | Medium-low as one narrow timing point in derived-Simulink; useful for identifying event quantization, not controller confirmation. | "R049R supports treating binary release delay as a selector for the next eligible release event, not a smooth control knob. Next work should audit the event boundary or move to soft/ramped restoration." | "R049R proves controlled reentry is solved, proves hardware/HIL safety, validates a complete controller, or proves all binary timing refinements are useless globally." |

<!-- R049S_PR_ECB_RELEASE_EVENT_BOUNDARY -->

### C63 / R049S: sampled release-event boundary audit

| Claim | Evidence | Strength | Safe wording | Do not claim |
|---|---|---|---|---|
| R049S confirms the sampled-event signal model: with `Ts_ctrl=40 ns`, the active-row one-shot boundary lies between `1.615 us` and `1.616 us`; `1.615 us` maps to `1.655 us`, while `1.616-1.630 us` map to `1.695 us` with larger undershoot penalty. | `docs/pr_ecb_release_event_boundary_r049s.md`; `refine-logs/LOCAL_AUDIT_R049S_PR_ECB_RELEASE_EVENT_BOUNDARY_20260625.md`; `output/iqcot_r049s_release_event_boundary_audit.m`; `output/iqcot_r049s_waveform_metric_audit.py`; `output/cutload_pr_ecb_control/r049s_release_event_boundary_results_full.csv`; `output/cutload_pr_ecb_control/r049s_waveform_metric_summary.md`. Sample-quantized one-shot predictions matched simulation with effectively `0 ns` error. Recovery metrics split into two plateaus: `1.655 us` gives `+0.1244/-0.7873 mV`; `1.695 us` gives `+0.1365/-1.1109 mV`. Decision: `MODEL_REVISED`. | Medium as a targeted derived-Simulink timing audit validating the signal model; not controller confirmation. | "R049S supports replacing continuous binary-release tuning with a sampled-event release model and motivates soft/ramped restoration." | "R049S proves controlled reentry is solved, proves hardware/HIL safety, validates a complete controller, or proves the exact event boundary under all offsets and loads." |
