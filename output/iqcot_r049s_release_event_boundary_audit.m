function rows = iqcot_r049s_release_event_boundary_audit(runSimulink)
%IQCOT_R049S_RELEASE_EVENT_BOUNDARY_AUDIT Audit release event quantization.
%
% R049S keeps the R049N upstream-causal release interface and runs a targeted
% active-offset micro-audit around the R049P/R049R/R049Q transition:
%
%   offset = 0.105 us
%   Tphase_release_delay = 1.615, 1.616, 1.620, 1.625, 1.630 us
%
% The goal is not to improve the controller directly.  It validates the signal
% model update discovered by R049R: binary release timing is quantized by the
% controller sample/update event that drives one_shot_done.
...
setappdata(0, "iqcot_r049s_run_simulink", runSimulink);
run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");
runSimulink = getappdata(0, "iqcot_r049s_run_simulink");
rmappdata(0, "iqcot_r049s_run_simulink");
...
plan = buildPlan();
planPath = fullfile(chunkRoot, "r049s_release_event_boundary_plan.csv");
writetable(plan, planPath);

if ~runSimulink
    rows = plan;
    writeDryRunReport(chunkRoot, planPath, modelFile);
    fprintf("R049S_RELEASE_EVENT_BOUNDARY_PLAN=%s\n", planPath);
    fprintf("Dry run only. Run with iqcot_r049s_release_event_boundary_audit(true).\n");
    return;
end
...
for k = 1:height(plan)
    ...
    fprintf("R049S case %s: %s, offset %.3fus, release %.3fus\n", ...
    ...
    try
        ...
        warning("iqcot:R049SReleaseEventBoundaryCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(chunkRoot, "r049s_release_event_boundary_results_full.csv");
reportPath = fullfile(chunkRoot, "r049s_release_event_boundary_report_full.md");
writetable(rows, resultPath);
writeRunReport(rows, resultPath, reportPath, modelFile, common);
fprintf("R049S_RELEASE_EVENT_BOUNDARY_RESULTS=%s\n", resultPath);
fprintf("R049S_RELEASE_EVENT_BOUNDARY_REPORT=%s\n", reportPath);
fprintf("R049S_DECISION=%s\n", diagnoseDecision(rows));
end

function plan = buildPlan()
releaseDelays = [NaN; 1.615; 1.616; 1.620; 1.625; 1.630];
case_id = strings(numel(releaseDelays), 1);
controller = strings(numel(releaseDelays), 1);
role = strings(numel(releaseDelays), 1);
trigger_mode = strings(numel(releaseDelays), 1);
post_inhibit_window_us = 1.690 * ones(numel(releaseDelays), 1);

case_id(1) = "r049s_20A_off0p105_a0";
controller(1) = "A0_no_inhibit";
role(1) = "baseline_same_model";
trigger_mode(1) = "disabled_negative_window";
post_inhibit_window_us(1) = -0.001;
phase_release_delay_us = releaseDelays;
phase_release_delay_us(1) = 1.615;

for k = 2:numel(releaseDelays)
    tag = delayTag(releaseDelays(k));
    case_id(k) = "r049s_20A_off0p105_a2_rel" + tag;
    controller(k) = "A2_release_" + tag + "us";
    role(k) = "release_event_boundary_probe";
    trigger_mode(k) = "release_timing_" + tag + "us";
end

base_load_A = 40 * ones(numel(releaseDelays), 1);
target_load_A = 20 * ones(numel(releaseDelays), 1);
load_drop_A = base_load_A - target_load_A;
load_step_offset_us = 0.105 * ones(numel(releaseDelays), 1);
tau_ai_us = 1.25 * ones(numel(releaseDelays), 1);
selected_ref_slew_us = 60 * ones(numel(releaseDelays), 1);
tton_trunc_min_ns = 196.5 * ones(numel(releaseDelays), 1);
tton_trunc_window_us = -0.001 * ones(numel(releaseDelays), 1);
post_inhibit_delay_us = 0.070 * ones(numel(releaseDelays), 1);
objective = repmat("r049s_release_event_boundary_audit", numel(releaseDelays), 1);

plan = table(case_id, controller, objective, role, trigger_mode, base_load_A, ...
...
function tag = delayTag(value)
tag = "1p" + extractAfter(sprintf("%.3f", value), ".");
end
...
common.tsCtrl = evalin("base", "Ts_ctrl");
...
if isempty(dt) || any(isnan(dt))
    ...
    inhibitDuration = sum(dt .* (inhibitRaw(:) > 0.5));
    effectiveInhibitDuration = sum(dt .* (inhibitRaw(:) > 0.5 & oneShotDone(:) < 0.5));
end

wavePath = fullfile(dataRoot, string(spec.case_id) + "_r049s_release_event_boundary_wave.csv");
wave = table(1e6 * tRel, vout, qh4, req, allowCtrl, inhibitRaw, releaseClock, oneShotDone, ...
    'VariableNames', {'time_from_load_step_us','vout_V','qh4','req_global', ...
    'allow_controlled_reentry','inhibit_raw','release_clock','one_shot_done'});
...
s.inhibit_raw_duration_us = 1e6 * inhibitDuration;
s.effective_inhibit_duration_us = 1e6 * effectiveInhibitDuration;
s.req_edges_during_effective_inhibit = countReqEdgesDuringEffectiveInhibit(req, inhibitRaw, oneShotDone);
s.sample_quantized_one_shot_us = predictQuantizedOneShotUs(spec, common);
s.sample_quantization_error_ns = 1e3 * (s.one_shot_time_us - s.sample_quantized_one_shot_us);
s.wave_csv = string(wavePath);
row = struct2table(s, "AsArray", true);
end

function tUs = predictQuantizedOneShotUs(spec, common)
if spec.post_inhibit_window_us < 0
    tUs = NaN;
    return;
end
releaseAbs = spec.t_load_step_s + spec.phase_release_delay_s;
sampleIndex = ceil((releaseAbs - 1e-15) / common.tsCtrl) + 1;
oneShotAbs = sampleIndex * common.tsCtrl;
tUs = 1e6 * (oneShotAbs - spec.t_load_step_s);
end
...
s = struct("success", false, "error_message", "", ...
    ...
    "final_error_mV", N, "release_clock_time_us", N, ...
    "one_shot_time_us", N, "one_shot_edge_count", N, ...
    "inhibit_raw_duration_us", N, "effective_inhibit_duration_us", N, ...
    "req_edges_during_effective_inhibit", N, ...
    "sample_quantized_one_shot_us", N, "sample_quantization_error_ns", N, ...
    "wave_csv", "");
end
...
function n = countReqEdgesDuringEffectiveInhibit(req, inhibitRaw, oneShotDone)
active = req(:) > 0.5 & inhibitRaw(:) > 0.5 & oneShotDone(:) < 0.5;
...
function decision = diagnoseDecision(rows)
if any(~rows.success)
    decision = "IMPLEMENTATION_ISSUE";
elseif numel(unique(rows.one_shot_time_us(rows.success & rows.one_shot_edge_count > 0))) < height(rows) - 1
...
reportPath = fullfile(chunkRoot, "r049s_release_event_boundary_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049S Release-Event Boundary Dry Run\n\n");
fprintf(fid, "- Model: `%s`\n", modelFile);
fprintf(fid, "- Plan: `%s`\n", planPath);
fprintf(fid, "- Offset: `0.105 us`.\n");
fprintf(fid, "- Release delays: `1.615, 1.616, 1.620, 1.625, 1.630 us`.\n");
end

function writeRunReport(rows, resultPath, reportPath, modelFile, common)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049S PR-ECB Release-Event Boundary Audit\n\n");
fprintf(fid, "- Model: `%s`\n", modelFile);
fprintf(fid, "- Results: `%s`\n", resultPath);
fprintf(fid, "- Ts_ctrl: `%.3f ns`\n\n", 1e9 * common.tsCtrl);
fprintf(fid, "| case | ctrl | release us | one-shot us | predicted us | err ns | peak mV | undershoot mV | eff inhibit us |\n");
fprintf(fid, "|---|---|---:|---:|---:|---:|---:|---:|---:|\n");
for k = 1:height(rows)
    if rows.success(k)
        fprintf(fid, "| %s | %s | %.3f | %.4f | %.4f | %.2f | %.4f | %.4f | %.4f |\n", ...
            rows.case_id(k), rows.controller(k), rows.phase_release_delay_us(k), ...
            rows.one_shot_time_us(k), rows.sample_quantized_one_shot_us(k), ...
            rows.sample_quantization_error_ns(k), rows.delta_v_actual_peak_mV(k), ...
            rows.secondary_undershoot_mV(k), rows.effective_inhibit_duration_us(k));
    else
