function rows = iqcot_r049p_pr_ecb_release_midpoint_audit(runSimulink)
%IQCOT_R049P_PR_ECB_RELEASE_MIDPOINT_AUDIT Test one intermediate binary release.
%
% R049P keeps the R049N upstream-causal release interface and tests exactly
% one intermediate binary release delay:
%
...
setappdata(0, "iqcot_r049p_run_simulink", runSimulink);
run("E:\Desktop\4cot\versions\v0027_20260611_135822_iqcot_optimized_final_cn_docs\init_four_phase_cot_sync.m");
runSimulink = getappdata(0, "iqcot_r049p_run_simulink");
rmappdata(0, "iqcot_r049p_run_simulink");
...
plan = buildPlan();
planPath = fullfile(chunkRoot, "r049p_release_midpoint_plan.csv");
writetable(plan, planPath);

if ~runSimulink
    rows = plan;
    writeDryRunReport(chunkRoot, planPath, modelFile);
    fprintf("R049P_RELEASE_MIDPOINT_PLAN=%s\n", planPath);
    fprintf("Dry run only. Run with iqcot_r049p_pr_ecb_release_midpoint_audit(true).\n");
    return;
end
...
for k = 1:height(plan)
    ...
    fprintf("R049P case %s: %s, offset %.3fus, release %.3fus\n", ...
    ...
    try
        ...
        warning("iqcot:R049PReleaseMidpointCaseFailed", "Case failed: %s", compactErrorReport(ME));
    end
end

resultPath = fullfile(chunkRoot, "r049p_release_midpoint_results_full.csv");
reportPath = fullfile(chunkRoot, "r049p_release_midpoint_report_full.md");
writetable(rows, resultPath);
writeRunReport(rows, resultPath, reportPath, modelFile);
fprintf("R049P_RELEASE_MIDPOINT_RESULTS=%s\n", resultPath);
fprintf("R049P_RELEASE_MIDPOINT_REPORT=%s\n", reportPath);
end

function plan = buildPlan()
case_id = [
    "r049p_20A_off0p050_a0";
    "r049p_20A_off0p050_a2_rel1p600";
    "r049p_20A_off0p105_a0";
    "r049p_20A_off0p105_a2_rel1p600"
];
controller = [
    "A0_no_inhibit";
    "A2_release_1p600us";
...
wavePath = fullfile(dataRoot, string(spec.case_id) + "_r049p_release_midpoint_wave.csv");
...
reportPath = fullfile(chunkRoot, "r049p_release_midpoint_dryrun_report.md");
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049P Release-Midpoint Dry Run\n\n");
fprintf(fid, "- Model: `%s`\n", modelFile);
fprintf(fid, "- Plan: `%s`\n", planPath);
fprintf(fid, "- Release delay: `1.600 us`.\n");
end

function writeRunReport(rows, resultPath, reportPath, modelFile)
fid = fopen(reportPath, "w");
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, "# R049P PR-ECB Release-Midpoint Audit\n\n");
fprintf(fid, "- Model: `%s`\n", modelFile);
