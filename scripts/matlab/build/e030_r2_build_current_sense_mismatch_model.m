function modelFile = e030_r2_build_current_sense_mismatch_model(variant)
%E030_R2_BUILD_CURRENT_SENSE_MISMATCH_MODEL Build E030-R2 current-sense mismatch models.
...
if variant ~= "R2-C0"
    addR2TonProjectionController(modelName);
end
...
function modelName = r2ModelName(variant)
if variant == "R2-C0"
    modelName = "E030_R2_C0_current_sense_from_ideal_iqcot_20260629";
elseif variant == "R2-C1"
    modelName = "E030_R2_C1_current_sense_from_ideal_iqcot_20260629";
elseif variant == "R2-C4a"
    modelName = "E030_R2_C4a_current_sense_from_ideal_iqcot_20260629";
elseif variant == "R2-C4c"
    modelName = "E030_R2_C4c_current_sense_from_ideal_iqcot_20260629";
else
...
set_param(ccsPath, "Name", "E030_R2_Load_Current_Source");
ccsPath = modelName + "/E030_R2_Load_Current_Source";
set_param(ccsPath, "Position", oldPosition, "Source_Type", "DC", ...
    "Amplitude", "0", "Measurements", "None");

stepBlock = modelName + "/E030_R2_LoadCurrentStep";
...
constBlock = modelName + "/E030_R2_ActivePhaseSet";
termBlock = modelName + "/E030_R2_ActivePhaseSet_Term";
...
for phase = 1:4
    fromBlock = modelName + "/E030_R2_IL_Sense_From" + phase;
    gainBlock = modelName + "/E030_R2_IL_Sense_Gain_Block" + phase;
    termBlock = modelName + "/E030_R2_IL_Sense_Term" + phase;
    deleteBlockIfExists(termBlock);
    ...
    add_block("simulink/Math Operations/Gain", gainBlock, ...
        "Gain", "E030_R2_IL_Sense_Gain" + phase, ...
...
fromBlock = modelName + "/E030_R2_Log_" + safeName + "_From";
termBlock = modelName + "/E030_R2_Log_" + safeName + "_Term";
deleteBlockIfExists(termBlock);
deleteBlockIfExists(fromBlock);
add_block("simulink/Signal Routing/From", fromBlock, ...
...
function addR2TonProjectionController(modelName)
ctrlBlock = modelName + "/E030_R2_TonProjection_Controller";
...
setR2TonProjectionScript(ctrlBlock);

clockBlock = modelName + "/E030_R2_TonProjection_Clock";
deleteBlockIfExists(clockBlock);
add_block("simulink/Sources/Clock", clockBlock, "Position", [3600 1180 3640 1210]);

consts = [
    "E030_R2_TonDiff_Enable", "E030_R2_K_T", "E030_R2_T_Trim_Max", ...
    "E030_R2_Current_Deadband", "E030_R2_Projection_Mode", "E030_R2_Vref", ...
    "E030_R2_V_Error_Budget", "E030_R2_V_Error_Hard_Limit", ...
    "E030_R2_Min_Scale", "E030_R2_Ripple_Budget", ...
    "E030_R2_Ripple_Hard_Limit", "E030_R2_Ripple_Window"];
...
for phase = 1:4
    ...
    connectBlocks(modelName + "/E030_R2_IL_Sense_Gain_Block" + phase, 1, ctrlBlock, 4 + phase);
end
...
termBlock = modelName + "/E030_R2_" + signalName + "_Term";
deleteBlockIfExists(termBlock);
...
clockBlock = modelName + "/E030_R2_TonActual_Clock";
deleteBlockIfExists(clockBlock);
add_block("simulink/Sources/Clock", clockBlock, "Position", [3720 620 3760 650]);

for phase = 1:4
    estimator = modelName + "/E030_R2_TonActual" + phase;
    termBlock = modelName + "/E030_R2_TonActual" + phase + "_Term";
    deleteBlockIfExists(termBlock);
...
function setR2TonProjectionScript(blockPath)
script = [
"function [ton1o,ton2o,ton3o,ton4o,trim1,trim2,trim3,trim4,clamp_flag,fallback_flag,scale,voltage_scale,ripple_scale] = R2_ton_projector(ton1,ton2,ton3,ton4,il1,il2,il3,il4,vout,t,enable,K_T,T_trim_max,deadband,mode,Vref,Verr_budget,Verr_hard,min_scale,ripple_budget,ripple_hard,ripple_window)"
"%#codegen"
...
"    fallback_flag = double(scale <= (min_scale + 1.0e-9));"
"else"
"    trim = zeros(1,4);"
"    scale = 1.0;"
"    clamp_flag = 0.0;"
...
setChartScriptAndTypes(blockPath, script, ...
    ...
    "ripple_hard", "ripple_window", "ton1o", "ton2o", "ton3o", "ton4o", ...
