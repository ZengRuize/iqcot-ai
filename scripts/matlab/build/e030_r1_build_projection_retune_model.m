function modelFile = e030_r1_build_projection_retune_model(variant)
%E030_R1_BUILD_PROJECTION_RETUNE_MODEL Build E030-R1 projection-retune models.
...
if variant ~= "R1-C0"
    addR1TonProjectionController(modelName);
end
...
function modelName = r1ModelName(variant)
if variant == "R1-C0"
    modelName = "E030_R1_C0_projection_retune_from_ideal_iqcot_20260629";
elseif variant == "R1-C1"
    modelName = "E030_R1_C1_projection_retune_from_ideal_iqcot_20260629";
elseif variant == "R1-C4a"
    modelName = "E030_R1_C4a_projection_retune_from_ideal_iqcot_20260629";
elseif variant == "R1-C4b"
    modelName = "E030_R1_C4b_projection_retune_from_ideal_iqcot_20260629";
elseif variant == "R1-C4c"
    modelName = "E030_R1_C4c_projection_retune_from_ideal_iqcot_20260629";
elseif variant == "R1-C4d"
    modelName = "E030_R1_C4d_projection_retune_from_ideal_iqcot_20260629";
else
...
set_param(ccsPath, "Name", "E030_R1_Load_Current_Source");
ccsPath = modelName + "/E030_R1_Load_Current_Source";
set_param(ccsPath, "Position", oldPosition, "Source_Type", "DC", ...
    "Amplitude", "0", "Measurements", "None");

stepBlock = modelName + "/E030_R1_LoadCurrentStep";
...
constBlock = modelName + "/E030_R1_ActivePhaseSet";
termBlock = modelName + "/E030_R1_ActivePhaseSet_Term";
...
fromBlock = modelName + "/E030_R1_Log_" + safeName + "_From";
termBlock = modelName + "/E030_R1_Log_" + safeName + "_Term";
deleteBlockIfExists(termBlock);
...
function addR1TonProjectionController(modelName)
ctrlBlock = modelName + "/E030_R1_TonProjection_Controller";
...
setR1TonProjectionScript(ctrlBlock);

clockBlock = modelName + "/E030_R1_TonProjection_Clock";
deleteBlockIfExists(clockBlock);
add_block("simulink/Sources/Clock", clockBlock, "Position", [3600 1180 3640 1210]);

consts = [
    "E030_R1_TonDiff_Enable", "E030_R1_K_T", "E030_R1_T_Trim_Max", ...
    "E030_R1_Current_Deadband", "E030_R1_Projection_Mode", "E030_R1_Vref", ...
    "E030_R1_V_Error_Budget", "E030_R1_V_Error_Hard_Limit", ...
    "E030_R1_Min_Scale", "E030_R1_Ripple_Budget", ...
    "E030_R1_Ripple_Hard_Limit", "E030_R1_Ripple_Window"];
...
for phase = 1:4
    ilBlock = modelName + "/E030_R1_TonProj_IL" + phase;
...
for phase = 1:4
    ...
    connectBlocks(modelName + "/E030_R1_TonProj_IL" + phase, 1, ctrlBlock, 4 + phase);
end
...
termBlock = modelName + "/E030_R1_" + signalName + "_Term";
deleteBlockIfExists(termBlock);
...
clockBlock = modelName + "/E030_R1_TonActual_Clock";
deleteBlockIfExists(clockBlock);
add_block("simulink/Sources/Clock", clockBlock, "Position", [3720 620 3760 650]);

for phase = 1:4
    estimator = modelName + "/E030_R1_TonActual" + phase;
    termBlock = modelName + "/E030_R1_TonActual" + phase + "_Term";
    deleteBlockIfExists(termBlock);
...
function setR1TonProjectionScript(blockPath)
script = [
"function [ton1o,ton2o,ton3o,ton4o,trim1,trim2,trim3,trim4,clamp_flag,fallback_flag,scale,voltage_scale,ripple_scale] = r1_ton_projector(ton1,ton2,ton3,ton4,il1,il2,il3,il4,vout,t,enable,K_T,T_trim_max,deadband,mode,Vref,Verr_budget,Verr_hard,min_scale,ripple_budget,ripple_hard,ripple_window)"
"%#codegen"
