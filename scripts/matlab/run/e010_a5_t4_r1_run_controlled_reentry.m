function rows = e010_a5_t4_r1_run_controlled_reentry(variants)
%E010_A5_T4_R1_RUN_CONTROLLED_REENTRY Run the T4-R1 burst-limiter revision.
% This is the smallest controlled-reentry revision after A5-T4 proxy failed.

if nargin < 1 || isempty(variants)
    variants = ["R1-T4a", "R1-T4b", "R1-T4c"];
end
variants = string(variants);
blocked = setdiff(variants, ["R1-T4a", "R1-T4b", "R1-T4c"]);
if ~isempty(blocked)
    error("This comparison may run only R1-T4a/R1-T4b/R1-T4c. Blocked: %s", strjoin(blocked, ", "));
end
...
experimentRoot = fullfile(projectRoot, "experiments", ...
    ...
    "R1_controlled_reentry_burst_limiter");
ensureDir(experimentRoot);

metricsCsv = fullfile(experimentRoot, "e010_a5_t4_r1_metrics.csv");
availabilityCsv = fullfile(experimentRoot, "e010_a5_t4_r1_signal_availability.csv");
schedulerCsv = fullfile(experimentRoot, "e010_a5_t4_r1_scheduler_audit.csv");
comparisonPath = fullfile(experimentRoot, "e010_a5_t4_r1_comparison.md");
waveformAuditPath = fullfile(experimentRoot, "e010_a5_t4_r1_waveform_audit.md");
summaryPath = fullfile(experimentRoot, "e010_a5_t4_r1_research_summary.md");
...
fprintf("E010_A5_T4_R1_METRICS=%s\n", metricsCsv);
fprintf("E010_A5_T4_R1_AVAILABILITY=%s\n", availabilityCsv);
fprintf("E010_A5_T4_R1_SCHEDULER_AUDIT=%s\n", schedulerCsv);
fprintf("E010_A5_T4_R1_COMPARISON=%s\n", comparisonPath);
fprintf("E010_A5_T4_R1_WAVEFORM_AUDIT=%s\n", waveformAuditPath);
fprintf("E010_A5_T4_R1_SUMMARY=%s\n", summaryPath);
disp(rows);
end
...
function rows = carryForwardRows()
rows = table();
refs = ["R1-C0", "R1-C4", "R1-T4proxy"];
for idx = 1:numel(refs)
    spec = referenceSpec(refs(idx));
    metrics = referenceMetrics(refs(idx));
    rows = appendTable(rows, rowFromMetrics(spec, metrics, true, ...
        "carry_forward_reference", "carry_forward"));
end
end

function spec = referenceSpec(name)
spec = struct();
spec.short_variant = string(name);
spec.Tton_trunc_min = NaN;
spec.Tton_trunc_window = NaN;
spec.E010_PulseInhibit_Time = NaN;
spec.burst_count_window = NaN;
spec.burst_pulse_limit_after_reentry = NaN;
spec.reentry_min_inter_pulse_spacing = NaN;
end

function m = referenceMetrics(name)
m = emptyMetrics();
m.logging_complete = true;
m.postprocess_complete = true;
m.success = true;
m.peak_overshoot_mV = baselinePeakOvershootMv();
m.delta_peak_overshoot_vs_C0_mV = 0;
m.delta_peak_overshoot_vs_C4_mV = 0;
m.peak_undershoot_mV = 0;
m.recovery_peak_2_12us_mV = baselineRecovery2To12Mv();
m.delta_recovery_peak_2_12us_vs_C0_mV = 0;
m.delta_recovery_peak_2_12us_vs_C4_mV = 0;
m.recovery_peak_12_40us_mV = baselineRecovery12To40Mv();
m.delta_recovery_peak_12_40us_vs_C0_mV = 0;
m.delta_recovery_peak_12_40us_vs_C4_mV = 0;
m.settling_time_us = NaN;
m.final_Vout_error_mV = 2.97792718436692;
m.Vout_ripple_pp_mV = 1.01145403542935;
m.Ton_trunc_count = 0;
m.Ton_trunc_min_ns = NaN;
m.Ton_saved_ns = 0;
m.Tton_trunc_window_us = NaN;
m.pulse_inhibit_count = 0;
m.inhibit_time_us = NaN;
m.REQ_reject_count = 0;
m.REQ_reject_reason = "0";
m.area_hold_count = 0;
m.area_reset_count = 0;
m.area_bleed_count = 0;
m.area_int_max = NaN;
m.area_int_at_reentry = NaN;
m.area_int_reentry_clamp_count = 0;
m.first_reentry_time_us = NaN;
m.first_reentry_phase = NaN;
m.first_reentry_Ton_ns = NaN;
m.burst_count_window_us = NaN;
m.burst_pulse_count_after_reentry = NaN;
m.burst_pulse_limit_after_reentry = NaN;
m.burst_limiter_clamp_count = 0;
m.burst_limiter_release_count = 0;
m.burst_limiter_fallback_reason = "0";
m.reentry_min_inter_pulse_spacing_us = NaN;
m.actual_min_inter_pulse_spacing_us = NaN;
m.recovery_Ton_ramp_usage = 0;
m.REQ_count = 149;
m.accepted_REQ_count = 149;
m.dropped_REQ_count = 0;
m.phase_order_error_rate = 0;
m.real_max_current_imbalance_A = 0.1241687110781;
m.real_rms_current_imbalance_A = 0.0896981295350865;
...
if name == "R1-T4proxy"
    ...
    m.delta_recovery_peak_2_12us_vs_C0_mV = ...
        m.recovery_peak_2_12us_mV - baselineRecovery2To12Mv();
    m.delta_recovery_peak_2_12us_vs_C4_mV = ...
        m.delta_recovery_peak_2_12us_vs_C0_mV;
    m.recovery_peak_12_40us_mV = 3.53369510017454;
    m.delta_recovery_peak_12_40us_vs_C0_mV = ...
        m.recovery_peak_12_40us_mV - baselineRecovery12To40Mv();
    m.delta_recovery_peak_12_40us_vs_C4_mV = ...
        m.delta_recovery_peak_12_40us_vs_C0_mV;
    m.final_Vout_error_mV = 2.96743257084865;
    m.Vout_ripple_pp_mV = 1.23407330907543;
    m.Ton_trunc_min_ns = 60;
    m.Ton_saved_ns = 136.5;
    m.Tton_trunc_window_us = 2;
    m.inhibit_time_us = 3;
    m.area_hold_count = 1;
    m.area_reset_count = 2;
    m.area_bleed_count = 1;
    m.area_int_max = 2.54091808247639e-09;
    m.area_int_at_reentry = 1.05489952351646e-09;
    m.first_reentry_time_us = 3.28200000000348;
    m.first_reentry_phase = 4;
    m.first_reentry_Ton_ns = 159.999999999971;
    m.burst_count_window_us = 2;
    m.burst_pulse_count_after_reentry = 5;
    m.burst_pulse_limit_after_reentry = 2;
    m.real_max_current_imbalance_A = 0.152764461444352;
    m.real_rms_current_imbalance_A = 0.116008971912039;
    m.guard_pass = false;
...
spec.E010A5_R1_BurstLimiter_Enable = 1;
spec.burst_count_window = 2.0e-6;
spec.burst_pulse_limit_after_reentry = 2;
spec.reentry_min_inter_pulse_spacing = 0.4e-6;
spec.E010A5_R1_AreaClamp_Enable = 0;
spec.E010A5_R1_TonRamp_Enable = 0;
spec.E010A5_R1_TonRamp_StartDelay = 3.0e-6;
spec.E010A5_R1_TonRamp_Window = 1.2e-6;
spec.E010A5_R1_FirstReentryTonLimit = 160e-9;

if variant == "R1-T4a"
    spec.variant = "R1-T4a_explicit_burst_limiter";
    spec.build_variant = "A5-T4-R1a";
    spec.file_prefix = "e010_a5_t4_r1_t4a_burst_limiter_40A_to_1A";
    spec.report_label = "R1-T4a explicit burst limiter";
elseif variant == "R1-T4b"
    spec.variant = "R1-T4b_burst_limiter_area_clamp";
    spec.build_variant = "A5-T4-R1b";
    spec.file_prefix = "e010_a5_t4_r1_t4b_burst_area_clamp_40A_to_1A";
    spec.report_label = "R1-T4b burst limiter plus area-int reentry clamp";
    spec.E010A5_R1_AreaClamp_Enable = 1;
elseif variant == "R1-T4c"
    spec.variant = "R1-T4c_burst_area_clamp_ton_ramp";
    spec.build_variant = "A5-T4-R1c";
    spec.file_prefix = "e010_a5_t4_r1_t4c_burst_clamp_ton_ramp_40A_to_1A";
    spec.report_label = "R1-T4c burst limiter plus area clamp and Ton ramp";
    spec.E010A5_R1_AreaClamp_Enable = 1;
    spec.E010A5_R1_TonRamp_Enable = 1;
else
    error("Unknown E010-A5-T4-R1 variant: %s", variant);
end
end
...
in = in.setVariable("E010A5_R1_BurstLimiter_Enable", ...
    spec.E010A5_R1_BurstLimiter_Enable);
in = in.setVariable("E010A5_R1_BurstWindow", spec.burst_count_window);
in = in.setVariable("E010A5_R1_BurstLimit", ...
    spec.burst_pulse_limit_after_reentry);
in = in.setVariable("E010A5_R1_MinInterPulseSpacing", ...
    spec.reentry_min_inter_pulse_spacing);
in = in.setVariable("E010A5_R1_AreaClamp_Enable", ...
    spec.E010A5_R1_AreaClamp_Enable);
in = in.setVariable("E010A5_R1_TonRamp_Enable", ...
    spec.E010A5_R1_TonRamp_Enable);
in = in.setVariable("E010A5_R1_TonRamp_StartDelay", ...
    spec.E010A5_R1_TonRamp_StartDelay);
in = in.setVariable("E010A5_R1_TonRamp_Window", ...
    spec.E010A5_R1_TonRamp_Window);
in = in.setVariable("E010A5_R1_FirstReentryTonLimit", ...
    spec.E010A5_R1_FirstReentryTonLimit);
...
metrics.area_int_reentry_clamp_count = maxOptionalSignal(logs, ...
    "burst_limiter_clamp_count", spec.t_load_step, spec.stop_time);
...
metrics.burst_limiter_clamp_count = maxOptionalSignal(logs, ...
    "burst_limiter_clamp_count", spec.t_load_step, spec.stop_time);
metrics.burst_limiter_release_count = maxOptionalSignal(logs, ...
    "burst_limiter_release_count", spec.t_load_step, spec.stop_time);
metrics.burst_limiter_fallback_reason = maxOptionalSignal(logs, ...
    "burst_limiter_fallback_reason", spec.t_load_step, spec.stop_time);
metrics.reentry_min_inter_pulse_spacing_us = ...
    1e6 * spec.reentry_min_inter_pulse_spacing;
metrics.actual_min_inter_pulse_spacing_us = actualMinAcceptedSpacingUs(logs, ...
    reentryTime, reentryTime + spec.burst_count_window);
metrics.recovery_Ton_ramp_usage = 1e9 * maxOptionalVector(logs, ...
    "recovery_Ton_ramp_usage", spec.t_load_step, spec.stop_time);
...
row.area_int_reentry_clamp_count = m.area_int_reentry_clamp_count;
...
row.burst_count_window_us = m.burst_count_window_us;
...
row.burst_limiter_clamp_count = m.burst_limiter_clamp_count;
row.burst_limiter_release_count = m.burst_limiter_release_count;
row.burst_limiter_fallback_reason = string(m.burst_limiter_fallback_reason);
row.reentry_min_inter_pulse_spacing_us = m.reentry_min_inter_pulse_spacing_us;
row.actual_min_inter_pulse_spacing_us = m.actual_min_inter_pulse_spacing_us;
row.recovery_Ton_ramp_usage = m.recovery_Ton_ramp_usage;
...
names = ["peak_overshoot_mV", "peak_undershoot_mV", ...
    ...
    "area_int_reentry_clamp_count", ...
    "first_reentry_time_us", "first_reentry_phase", "first_reentry_Ton_ns", ...
    "burst_count_window_us", "burst_pulse_count_after_reentry", ...
    "burst_pulse_limit_after_reentry", "burst_limiter_clamp_count", ...
    "burst_limiter_release_count", "reentry_min_inter_pulse_spacing_us", ...
    "actual_min_inter_pulse_spacing_us", "recovery_Ton_ramp_usage", ...
...
m.burst_limiter_fallback_reason = "NaN";
m.current_limit_hit = false;
m.undershoot_budget_violation = false;
m.late_settling_guard_violation = false;
...
names = [
    ...
    "controlled_reentry_active", "burst_limiter_state", ...
    "area_int_reentry_clamp", "burst_limiter_clamp_count", ...
    "burst_limiter_release_count", "burst_limiter_fallback_reason", ...
    "recovery_Ton_ramp_usage"];
end
...
for name = ["Ton_trunc_i", "Ton_saved_i", "Lambda_i", "area_int_i", ...
        ...
        "burst_pulse_count_after_reentry", "controlled_reentry_active", ...
        "burst_limiter_state", "area_int_reentry_clamp", ...
        "burst_limiter_clamp_count", "burst_limiter_release_count", ...
        "burst_limiter_fallback_reason", "recovery_Ton_ramp_usage"]
    out.(name) = interpOptionalSignal(logs, name, timeGrid);
end
out.actual_inter_pulse_spacing = localInterPulseSpacingSeries(logs, timeGrid, spec);
...
fprintf(fid, "# E010-A5-T4-R1 Candidate Comparison\n\n");
fprintf(fid, "Date: %s\n\n", reportDate());
fprintf(fid, "## Scope\n\n");
fprintf(fid, "This R1 comparison carries forward `R1-C0`, `R1-C4`, and `R1-T4proxy`, then runs only `R1-T4a`, `R1-T4b`, and `R1-T4c` for the fixed external `40A -> 1A` load-current drop. It does not enable active Lambda, active-phase add/shed, DCR mismatch, current-sense mismatch, or a broader load-step grid.\n\n");
fprintf(fid, "The baseline references are the already confirmed A5-C0/A5-C4 audit values: peak overshoot `%.6g mV`, recovery peak 2-12 us `%.6g mV`, and recovery peak 12-40 us `%.6g mV`. The carried-forward T4proxy keeps the previous partial recovery improvement but fails burst guard with `5 / 2` pulses.\n\n", ...
    baselinePeakOvershootMv(), baselineRecovery2To12Mv(), baselineRecovery12To40Mv());
fprintf(fid, "## Candidate Settings\n\n");
fprintf(fid, "- `R1-T4a`: explicit burst limiter and conservative inter-pulse spacing in the IQCOT request-enable path.\n");
fprintf(fid, "- `R1-T4b`: T4a plus area-int reentry clamp while burst limiting is active.\n");
fprintf(fid, "- `R1-T4c`: T4b plus conservative recovery Ton ramp.\n\n");
fprintf(fid, "## Metrics Snapshot\n\n");
writeMetricsTable(fid, rows);
fprintf(fid, "## Guard Summary\n\n");
fprintf(fid, "| Variant | Formal positive metric lower | Guard pass | Dropped REQ | Phase err | Current limit | Undershoot fail | Burst count/limit | Late settling fail | Classification |\n");
fprintf(fid, "|---|---:|---:|---:|---:|---:|---:|---:|---:|---|\n");
...
if classification == "MODEL_REVISED"
    fprintf(fid, "R1-T4a/b/c suppress positive overshoot/recovery peaks only because Vout falls into a severe undershoot and final-error failure. This is not accepted as a useful recovery improvement. The burst counter also remains `5 / 2`, so the count-based limiter did not close the original guard failure.\n\n");
end
fprintf(fid, "Audit note: the phase-order guard in `e010_a5_t4_r1_metrics.csv` is computed from the accepted-REQ event sequence. The optional sampled model signal `phase_order_error` is retained in the scheduler audit as `model_phase_order_error_sample` and is not used as the pass/fail event-sequence guard because it can pulse at sampled edge times.\n\n");
...
fprintf(fid, "\n## Wave Samples\n\n");
for idx = 1:height(simRows)
    prefix = filePrefixForVariant(simRows.variant(idx));
    fprintf(fid, "- `%s`: `%s`\n", simRows.variant(idx), ...
        slashPath(fullfile(experimentRoot, prefix + "_wave_sample.csv")));
end
fprintf(fid, "\n## Interpretation\n\n");
fprintf(fid, "`%s`: waveform logging supports only this local fixed-four-phase severe-drop R1 comparison. Active Lambda, active-phase interaction, broad robustness, and hardware/HIL claims remain forbidden.\n", classification);
end
...
cleanup = onCleanup(@() fclose(fid));
fprintf(fid, "# E010-A5-T4-R1 Research Summary\n\n");
fprintf(fid, "Date: %s\n\n", reportDate());
fprintf(fid, "## Result\n\n");
fprintf(fid, "`%s`\n\n%s\n\n", classification, detail);
...
bestVariant = "none";
candidateRows = rows(startsWith(rows.variant, "R1-T4"), :);
if isempty(candidateRows) || any(~candidateRows.success) || ...
    ...
    detail = "R1 comparison failed because at least one run, required signal, or hard metric was unavailable. Do not claim A5 improvement.";
    return;
end
improved = false(height(candidateRows), 1);
for idx = 1:height(candidateRows)
    improved(idx) = improvesBaseline(candidateRows(idx, :));
end
...
if any(safeImproved)
    ...
    detail = "At least one R1-T4 candidate improved peak or recovery versus the confirmed A5-C0/A5-C4 severe-drop baseline while satisfying the configured burst/reentry guard set.";
elseif any(improved)
    classification = "MODEL_REVISED";
    candidates = candidateRows(improved, :);
    bestVariant = candidates.variant(1);
    detail = "R1 preserves the earlier T4proxy partial recovery reference but the new burst-limited candidates fail the guard set. R1-T4a/b/c suppress positive peaks by driving severe undershoot/final-error collapse and still leave burst count above the limit.";
else
    classification = "CLAIM_DOWNGRADED";
    detail = "No R1-T4 candidate safely improved the confirmed A5-C0/A5-C4 severe 40A -> 1A boundary. A5 requires further revision before any severe-drop improvement claim.";
end
end
...
hardSignals = [
    ...
    "Ton_cmd1", "Ton_cmd2", "Ton_cmd3", "Ton_cmd4", ...
    "Ton_actual1", "Ton_actual2", "Ton_actual3", "Ton_actual4", ...
    "area_int_i", "phase_idx", "controlled_reentry_active", ...
    "burst_limiter_state", "burst_limiter_clamp_count", ...
    "burst_limiter_release_count"];
complete = true;
...
function values = localInterPulseSpacingSeries(logs, timeGrid, spec)
times = allAcceptedTimes(logs, spec.t_load_step, spec.stop_time);
values = NaN(size(timeGrid));
if numel(times) < 2
    return;
end
for idx = 1:numel(timeGrid)
    past = times(times <= timeGrid(idx));
    if numel(past) >= 2
        values(idx) = past(end) - past(end - 1);
    end
end
values = 1e6 * values;
end
...
columns = {'variant','success', ...
    ...
    'area_bleed_count','area_int_max','area_int_at_reentry', ...
    'area_int_reentry_clamp_count','first_reentry_time_us', ...
    'first_reentry_phase','first_reentry_Ton_ns', ...
    'burst_count_window_us','burst_pulse_count_after_reentry', ...
    'burst_pulse_limit_after_reentry','burst_limiter_clamp_count', ...
    'burst_limiter_release_count','burst_limiter_fallback_reason', ...
...
variant = string(variant);
if variant == "R1-T4a"
    prefix = "e010_a5_t4_r1_t4a_burst_limiter_40A_to_1A";
elseif variant == "R1-T4b"
    prefix = "e010_a5_t4_r1_t4b_burst_area_clamp_40A_to_1A";
elseif variant == "R1-T4c"
    prefix = "e010_a5_t4_r1_t4c_burst_clamp_ton_ramp_40A_to_1A";
else
